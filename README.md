...
- C137 acceptance bundle v3 / protocol v5 / relationship ranking report v3 已接入 formal blind matrix provenance v2，同时继续要求 performance report v3 / raw schema v2。协议固定 formal/plan/native receipt 版本、物理候选全集、axis、visual、global K、shard-invariant score contract 与 exhaustive coverage；协调器对完整 manifest 只预检一次，再把 query×candidate 矩阵确定性拆成“每侧最多 256、每 tile 最多 256 pair”的原生任务。每个 pair 在 tile-local assignment 和 fine 之前发布独立关系分数，聚合器必须收齐每个矩阵 cell 后从全部分数重算全局 Top-K，并按 manifest 候选序号稳定破同分；禁止拼接局部 Top-K。第一个 completed tile 会固定 actual execution identity，之后任何 native build、FFmpeg/FFprobe 或 CPU/CUDA/fallback 漂移都会令整个 matrix 失败关闭并丢弃已完成 envelope。旧 bundle v2/protocol v4/ranking v2/formal v1/native receipt v1 均严格拒绝。任一 tile 失败、取消、漏 cell、重复 receipt/job、参数漂移、执行身份漂移或候选同文件异流时不会生成 provenance。内容闭环后仍固定 `self-consistent-no-native-authority`：当前仍缺少外部 plan 签名、native attestation、一次性 challenge、有状态防重放账本及 modality/calibration 来源，因此正式门禁继续返回 `incomplete-evidence`。普通产品单次原生任务仍以 256 pair 为硬上限；formal 的跨 tile 全局关系排名不等于跨 tile 重做项目级 global assignment、fine TimeMap 或真正 same-segment many-to-many。
