...
- 本地音频对齐实验室：在 Tauri 桌面端调用用户本机 FFmpeg 抽取音频特征，生成 `AlignmentProposal`，支持后台进度、取消、任务日志、调参、音频候选边界细化和不确定区间展示、导入提案、按项目名导出提案和复核报告、应用前复核提示、带具体 ID 的异常提案应用阻断和一键应用。
...
`.danmaku-project.json` 保存项目 schema、媒体引用、XML 资源元数据、解析后的弹幕数据、片段、版本差异、同步线索、禁用弹幕、单条时间调整、时间轴视图和预览设置。视频内容不会嵌入项目文件。

当前项目文件 schema 为 v2。v2 片段源区间使用半开语义 `[sourceInMs, sourceOutMs)`；打开 v1 项目时会自动把旧闭区间边界上正好存在弹幕的片段迁移为 `sourceOutMs + 1`，并在状态栏提示旧版项目已升级及调整片段数量。保存项目时始终写入当前 schemaVersion。

打开项目时会校验弹幕资源、片段、版本差异、同步线索、整数毫秒时间字段、预览设置和时间轴视图，避免坏的非破坏性规则进入运行时状态。

资源栏“导出检查”会基于当前运行时状态生成导出前检查摘要，用于在保存、重开或导出前快速复核媒体重连、空片段、导入警告、重复 ID、负最终时间和失效引用；重复 ID、缺失资源片段、未放入时间轴资源、全部禁用片段、空片段、0ms 版本差异、媒体重连、导入警告、低置信同步线索、失效编辑引用与负最终时间会显示出现位置并写入可下载检查报告。缺失资源片段、失效禁用/单条微调引用可通过真实清理动作进入历史栈，仍可撤销。
...
- 左侧资源面板：默认显示弹幕素材、下一步操作和导出检查；高级工具展开后可使用 Emby 时长、人工整理规则、版本差异列表、疑似版本差异扫描、锚点校准、同步线索管理和视频对齐实验室。
...
2. 把弹幕素材放到时间轴。
3. 看预览，调整不准的地方。
4. 如果视频版本有差异，就标记当前视频哪里比完整版多了或少了多久。
5. 通过导出前检查后导出 XML；导出的 XML 会重新解析验证，原始 XML 不会被直接修改。
...
- 本地音频对齐依赖用户提供合法拥有或授权读取的完整版、当前视频和本机 FFmpeg；网页模式不会伪装成本地桌面能力。
