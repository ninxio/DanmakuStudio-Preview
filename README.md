...
- 本地音频对齐实验室：在 Tauri 桌面端调用用户本机 FFmpeg 抽取音频特征，生成 `AlignmentProposal`，支持后台进度、取消、任务日志、调参、目标本地路径复用、音频候选边界细化、不确定区间展示、候选版本差异逐条预览/修正/接受/跳过、导入提案、按项目名导出提案和复核报告、应用前复核提示、带具体 ID 的异常提案应用阻断和一键应用。
...
- 当前对齐算法是本地音频特征和动态规划候选生成，已对音频候选使用相邻匹配区间中点作为初始边界，并展示不确定区间；候选版本差异可以逐条预览、修正毫秒时间和差异时长、接受或跳过，接受后仍通过非破坏性版本差异规则生效。视觉对齐、特征缓存和更细粒度局部重匹配仍是后续增强。
...
- 完善音频对齐实用化：增加特征缓存，避免同一视频反复全量扫描；验证 Emby 授权流或低码率转码采样后，再接入 Emby 目标原片作为直接输入。
