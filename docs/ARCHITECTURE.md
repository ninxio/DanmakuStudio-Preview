...
音频特征缓存位于 Tauri 后端进程内，不写入 `.danmaku-project.json`。缓存 key 由规范化本地路径、文件大小、修改时间、FFmpeg 路径、采样率和特征窗口组成；匹配阈值和最小缺失时长变化时可复用同一组特征，换文件、换窗口或文件更新后会重新提取。任务日志和提案诊断会说明完整版/当前视频是缓存命中还是新提取。
