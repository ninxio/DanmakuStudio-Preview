...
- 左侧资源面板：默认显示弹幕素材、目标原片绑定、下一步操作和导出检查；高级工具展开后可使用 Emby 时长、人工整理规则、版本差异列表、疑似版本差异扫描、锚点校准、同步线索管理和视频对齐实验室。
...
3. 把弹幕素材放到时间轴。
4. 看预览，调整不准的地方。
5. 如果视频版本有差异，就标记当前视频哪里比完整版多了或少了多久。
6. 通过导出前检查后导出 XML；导出的 XML 会重新解析验证，原始 XML 不会被直接修改。
