...
- 为项目绑定目标原片：可把当前本地视频标记为目标，也可把授权 Emby 条目保存为目标原片引用。
...
- HTML Video 轻量预览：显示未导入、正在载入、可播放、格式不支持和需要重新连接等状态，支持播放/暂停、弹幕叠加、透明度调整，并可在当前播放点直接标记版本差异。
- 撤销、重做和常用快捷键。
...
`.danmaku-project.json` 保存项目 schema、媒体引用、目标原片绑定、XML 资源元数据、解析后的弹幕数据、片段、版本差异、同步线索、禁用弹幕、单条时间调整、对齐提案、时间轴视图和预览设置。视频内容不会嵌入项目文件。
...
`mediaBinding` 只保存恢复绑定所需的非敏感信息。本地绑定保存媒体 ID、文件名和显示名；Emby 绑定保存条目 ID、剧名、季集号、媒体源摘要、运行时长和服务器配置引用。Emby 密码、访问 token、临时播放 URL、本地视频对象 URL 和视频内容都不会写入项目文件。
...
- 当前没有接入 mpv，因此不承诺 MKV、外挂字幕高级渲染或硬解能力；HTML Video 播放失败时会明确提示改用 MP4/WebM 或等待后续启用 mpv 播放器。
