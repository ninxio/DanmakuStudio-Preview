...
- 播放预览：HTML Video 作为 MP4/WebM 轻量 fallback；桌面端配置 mpv 且目标原片有真实本地路径时可切到 mpv sidecar。预览会显示未导入、正在载入、可播放、格式不支持和需要重新连接等状态，支持播放/暂停、seek、弹幕叠加、透明度调整，并可在当前播放点直接标记版本差异。播放器会话状态栏会统一展示播放源、后端、播放状态、音轨、字幕轨、弹幕轨、缓存和下一步操作；双源对比状态会展示 B 站参考侧、目标原片侧、当前参考时间、映射后的目标时间和已应用补偿，不把尚未接入的能力伪装成可用。
- 撤销、重做和常用快捷键。
...
- 完善 mpv 体验：嵌入式窗口、状态恢复、更多错误诊断，以及未来的 Emby 授权流媒体播放验证。
