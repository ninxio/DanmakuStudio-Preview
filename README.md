...
- 原生 batch snapshot 现可给出完整 source-major `source×target` inventory、pair ordinal、每 pair 候选数/Top-10/决策候选和最终 shortlist，并在最终媒体身份复核后原子发布。blind N×M 编译链会显式固定关系查询轴和视觉模式，用两侧独立的 salted commitment 排列媒体；`caseIds` 只切 query 轴，`candidateCaseIds` 单独锁定完整候选全集，suite/projection digest 同时绑定两者。execution/receipt 的 strict contract 已移到领域层，raw prediction 和逐 case gold/Top-K 只能由通过校验的 completed full-Cartesian native receipt 确定性派生，调用方不能重新排序后重签。当前每侧不超过 64 个 distinct media、总数不超过 256 pair、候选数严格大于 K；正式视觉模式还要求两侧都预先固定非空视频流，禁止 `null/auto`。projection、raw、逐 case gold/rank、完整 manifest、execution suite 和 native receipt 只留在本机私有边界，公开报告仍仅保存纯聚合指标并固定为 `untrusted-self-consistent-evidence`；它不评测同一 pair 内定位窗口 Top-K、删减分类或边界准确率，也不是 release authority。
...
- 当前 release 已包含可选 CUDA/cuFFT 声谱后端；只有驱动、cuFFT、CUDA context 和真实 R2C smoke 全部通过才报告 GPU ready。短媒体共享声谱和长媒体 streaming coarse FFT 可走 NVIDIA GPU，Auto 失败会丢弃 GPU 中间结果并从头回退 CPU，强制 CUDA 则 fail-closed。FFmpeg `-vn` 音频解码、全文件 SHA-256、landmark 配对、edit-aware DP、边界精修和项目级 exact search 仍使用 CPU；NVDEC 只可能帮助独立视觉回退的帧解码，不能替代音频计算后端。
...
- C137 acceptance bundle v2 / protocol v4 / relationship ranking report v2 已把私有 blind structural provenance 接入正式聚合器，同时继续要求 performance report v3 / raw schema v2。协议预先锁定完整候选全集、query shards、axis、visual、K 和参数摘要；聚合器从完整 manifest 唯一重建 projection，严格复核 execution suite、completed native receipt、raw、gold aggregate 和全局 case 双射，再要求 ranking report 的 `decisionId/provenanceRef/case/gold/Top-K/verified` 逐条等于 native 派生结果。相同 case 即使更换 batch、axis 或 visual 也不能重复计数，旧 bundle v1/protocol v3/ranking v1 不能混入新语义。公开 aggregate evidence 不能塞进该私有字段。内容闭环通过后，`native-blind-ranking-provenance` 仍会明确显示 `self-consistent-no-native-authority`：当前证据包不含外部 plan 签名、native execution attestation、一次性 challenge、防重放账本及 modality/calibration 来源凭据，`external-trust-authority` 也继续未完成，因此正式门禁仍返回 `incomplete-evidence`。此外，当前 native 每侧 64-media 上限意味着 distinct 候选全集超过 64 时会诚实阻断；不得靠候选分片后拼接局部 Top-K 冒充全局排名。
...
- 在现有每媒体一次预处理、长参考流式索引和可选 CUDA/cuFFT FFT 之上，继续实现 pair-local 多窗口 Top-K、GPU 批量相关，以及同一内容片段可安全对应多个版本的受约束 many-to-many fine alignment；CPU 继续作为确定性基线、等价复核与自动回退。
