...
`c137BlindBatchEvidence.ts` 与 `c137BlindBatchBenchmark.ts` 提供独立的 blind N×M 组件证据链。调用方必须显式声明关系查询轴与实际视觉模式；媒体两侧分别用 manifest/domain salted commitment 排列，私有 projection 绑定完整内容身份和实际消费流但不暴露原始摘要或路径，候选侧 distinct 数必须严格大于 K。compiler 与 runner/native 共用每侧 64、总计 256 pair 的硬边界，并在任何路径 preflight 前拒绝超限 suite。视觉关闭时 video stream 被归一为 `null`，不能靠未消费的视频流虚增候选；formal visual 模式要求每个 selected source/target 预先固定非空视频流，禁止 `null/auto`。协调器先完成路径/身份/流 preflight，再发起一次完整 native batch；只有 completed receipt 通过严格 validator 后才揭示冻结 gold，任何 partial、失败、取消、重放或 projection 漂移都令 accuracy evidence 为空。projection、raw prediction、逐 case gold/rank、suite/manifest 标识和私有运行摘要不跨出本机编译边界；公开 DTO 只有聚合计数、准确率与误差分布，不能当作 native 签名或 release authority。输出固定为 `scope: "cross-media-relationship-and-known-pair-anchor-component"`、`releaseEligible: false`、`trustStatus: "untrusted-self-consistent-evidence"`。v1 只重算跨媒体关系 Top-1/Top-K、gold pair shortlist 命中和 mapped-anchor coverage/error，不评测同一 pair 内多窗口 Top-K、编辑分类、删减/插入 F1 或边界误差。
