...
原生 N×M batch 另有完整 source-major receipt：包含 source/target inventory、全部 pair、每 pair 的候选数、Top-10、决策候选、shortlist 状态和 TimeMap，并在最终身份复核后原子发布。该原始 receipt 仍含内容身份和流元数据，只供本机内部协调器使用，不是可直接分享的匿名报告。execution/receipt 的 exact contract、canonical digest 和 ranking 重算位于纯领域模块；私有 blind 编译边界用 salted media commitment、projection digest 和 raw prediction 绑定运行，且 raw 只能从整批/逐 pair completed 的 native receipt 唯一派生。`caseIds` 只切 query 轴，`candidateCaseIds` 锁定候选全集；关系轴、视觉模式、K 与两组 case 在 suite digest 中固定。公开报告不携带 projection、raw、逐 case ID/gold/pair 排名、media binding、suite/manifest 标识或私有运行摘要，只保留纯聚合指标。当前每侧最多 64 个媒体、总数最多 256 pair，候选数必须严格大于 K；视觉关闭时仅视频流不同不能冒充新候选，正式视觉模式则禁止 `null/auto` 流。公开结果仍固定为 `releaseEligible:false` / `untrusted-self-consistent-evidence`，也不评测同一 pair 内窗口 Top-K、编辑/删减分类或边界准确度。
...
完整 release 验收另需严格的 `C137AcceptanceBundle` 和外部信任。bundle v2 / protocol v4 / relationship report v2 已新增私有 formal blind provenance：完整 manifest、预注册 query shards 与 candidate universe、execution suite、native receipt、确定性 raw/aggregate 及 provenance root 会在本机 acceptance 内重新计算；每个 frozen case 全局只能出现一次，ranking report 必须用 `provenanceRef` 与派生 decision 逐条闭合。manifest/gold/media/parameters/provenance root 分别绑定 bundle、dataset approval、preflight、runner 与 prediction receipt，公开聚合报告不能代替该私有 envelope。performance report v3 仍只接受 formal raw schema v2，并独立检查实际媒体卷、Job memory、terminal cleanup 与 native attestation。当前内容闭环可以通过，但 `native-blind-ranking-provenance` 仍固定因 `self-consistent-no-native-authority` 而 incomplete：plan authority、native execution attestation、一次性 challenge、防重放账本和 modality/calibration 来源尚未实现；`external-trust-authority` 同样未完成。候选全集若超过当前每侧 64 distinct media 上限也会直接阻断，不能通过候选分片拼接局部 Top-K。安装包默认不携带审批白名单或可自行签发的外部 authority，因此正式结果继续是 `incomplete-evidence`。
