...
> 当前面向真实产品闭环的阶段顺序、数据规模和硬结束指标见
> [`C137-execution-plan.md`](./C137-execution-plan.md)。本文继续作为完整技术规范与历史验收清单。
...
这些能力解决的是“怎样盲跑生产算法、怎样安全拥有和清理媒体进程树、怎样把实际媒体卷、Job 成员内存和任务收尾绑定到性能记录、怎样防止改字符串/自摘要/跨 workload 复用冒充完整验收”，不代表真实准确率或正式性能验收已经通过。实际 `workload-media-volumes`、Job memory 与 terminal cleanup 三类 receipt 已完成严格生成和重算；raw v2 如实声明 `windows-job-object-working-set-v1`，应用自身 raw 中的 `assurance.attestation` 仍为空，必须由仓库外 authority v3 的动态进程证明补充。authority v3 已能闭合 `authenticode-artifact-attestation` 和软件 live-process `native-attestation`；当前没有正式 key/policy、受信代码签名证书、硬件 TPM/AIK 实例或由获批真实 calibration split 拟合的生产模型，产物继续固定为 `releaseEligible=false` 的 engineering raw。当前获授权且完成运行的真实冻结关系数仍为 0；真实校准模型批准、规定目标机的正式性能测量、20 套北极星长合集和真实媒体回归均未完成。
...
完整 C137 acceptance bundle 已定义 versioned protocol、数据审批/preflight/prediction receipts，以及 dataset、关系排名、TimeMap、校准、视觉回退、安全降级、北极星、性能、UI 和 release 原始报告。bundle v6 / protocol v8 / relationship report v3 / calibration report v2 另有私有 formal blind matrix provenance v3：plan 在运行前锁定完整物理 candidate universe、二维 query×candidate tiles、axis、visual、global K、score contract 与 exhaustive coverage；acceptance 从完整 manifest 重建每个 projection，严格复核 execution 路径、全文件身份、流、full-Cartesian pairs、completed native receipt v4、raw 与 provenance root，并从全部 matrix cell 重算 global Top-K。每个 frozen query 只能产生一次决策，relationship report 的 scope/score/K 与 `decisionId/provenanceRef/case/gold/Top-K/verified` 必须逐项等于派生结果；calibration report 的 Top-1/Top-2 score、margin、model digest 和 probability 必须逐项等于原生派生及批准模型插值。performance report v3 继续只接受 raw schema v2，并检查实际媒体卷、Job memory、terminal cleanup 与 native attestation。旧 bundle v5/protocol v7/calibration report v1、raw v1 和自建 trustContext 均不能晋级。

acceptance 不内置可自行放行的非空白名单。当前 `trustContext` 只保存调用方提供的 protocol、三类 receipt 和每份 raw report 的 canonical SHA-256；报告摘要从原始内容重算并排除自身 `evidenceDigest`，所以它可发现快照后的内容变化，却不能证明摘要由独立信任根签发。独立 `c137:authority` 工具和 authority v3 验证器要求私钥由 release 进程在仓库外持有，P-256 公钥和允许的 Authenticode signer 叶证书 SHA-256 由 out-of-band trust policy 固定。`inspect-native`、`attest` 和 `verify` 都会对指定 EXE 重新计算全文件摘要并调用 Windows Authenticode 信任验证；缺少有效 signer/时间戳、摘要不等于 formal execution identity、证书不在白名单或 verify 时换成另一 EXE 都失败。authority 在结果产生前签发一次性 256-bit nonce challenge，绑定 protocol、manifest、dataset、blind/performance plan、环境、release build 和参数；桌面进程用临时 Ed25519 会话绑定 challenge、PID、Windows 创建 FILETIME 和当前 EXE 摘要，只允许终态 native job/performance evidence 按精确 inventory 各封存一次。finalize 签名动态 evidence root；运行后 authority 再通过 OS 独立复核 live PID、启动时间、映像路径和映像摘要，把完整 acceptance bundle、formal provenance、performance raw、进程观察、live-process receipt 与 artifact attestation 一起签名。append-only ledger 为同一 challenge 依次写入唯一 `issued` / `consumed` 动作并签发 checkpoint，重复消费、跨 bundle 复用、过期、低于外部 minimum sequence 的回滚、未命中固定 checkpoint、伪密钥、跨进程替换和任一摘要漂移均被拒绝。

authority v3 proof 可解除外部 trust、blind plan、challenge freshness、replay-ledger、两个 Authenticode artifact 检查，以及 blind/performance 的软件 `native-attestation`；它仍不会解除 `native-blind-ranking-provenance`，也不会把缺失的真实数据或批准模型变出来。该证明使用进程内可导出的软件密钥和 Windows 进程观察，不等于 TPM/AIK 不可迁移密钥、硬件代码完整性或受保护进程。当前 release 本身也未签名，没有 production authority 私钥、公钥/checkpoint/signer 白名单或批准协议，所以默认应用继续 `incomplete-evidence`，不存在可自行生成并放行的 production envelope。
...
实际媒体卷、Job 成员内存和终态清理回执已经拆成独立、可重算的检查。应用生命周期内的 benchmark accounting Job 固定当前进程及其后续后代；每个 FFmpeg/FFprobe 同时进入自己的 kill-on-close 子 Job，形成可清理的嵌套成员关系。内存采样器从 accounting Job 的精确成员清单读取 working set，并要求采样前后成员集合一致、全部样本成功、全部 native job 覆盖完整且终态进程树归零；会话开始前若已有 Job 后代则直接拒绝，不允许把它们豁免成 baseline。raw v2 因而声明 `windows-job-object-working-set-v1`，Job memory receipt 可在严格重算后单项通过。authority v3 进一步把这些回执所在 bundle 与固定 signer 的 Authenticode EXE、同一 live process 的临时 Ed25519 challenge-response、OS 进程映像观察和精确动态 evidence inventory 绑定；在 production key/policy/signer、获批协议和规定 4 核目标机重复运行完成前，当前 raw 仍只能用于工程诊断。blind runner 和组件面板的协调器 wall time同样不能替代正式性能证据。
...
- [x] acceptance bundle v6 / protocol v8 / relationship report v3 / calibration report v2 与 performance report v3 只接受对应 formal schema；性能链独立检查当前 manifest、storage、Job memory、terminal cleanup 与 attestation，blind matrix 链直接重算 plan/projection/execution/native receipt v4/raw/global decision/Top-1/Top-2 margin/model probability；改字符串、自摘要、选择性漏报、重复 cell、local Top-K、自填概率、自建 trustContext 或跨 workload 重签均不能通过。
...
- [x] 为 formal blind plan 增加仓库外 P-256 authority 签名、一次性 challenge、有效期与 append-only 防重放账本；authority v3 独立复核 EXE 全文件摘要、Windows Authenticode、固定 signer/时间戳证书，并以进程内临时 Ed25519 + PID/Windows 创建 FILETIME/OS 映像观察绑定终态 blind/performance evidence inventory。伪密钥、过期、重复消费、跨 bundle、换 EXE/证书/进程、漏封、重复封存、重放、回滚和 v2 降级均失败关闭。
- [x] 同一 live native process 已接入软件 challenge-response 与 Windows 进程映像复核，正式 `native-blind-native-attestation` 和性能 `native-attestation` 可由有效 authority v3 proof 解除；该实现不是 TPM/AIK 或硬件不可迁移密钥，硬件级增强仍可后续追加。
- [ ] 用获批真实 calibration split 拟合、复核和签发生产模型；modality 已从同一 gold pair 的原生 TimeMap 锚点、双端流身份和 fine execution 唯一推导并与报告 exact binding，calibration 代码链也已把原生 Top-1/Top-2 score margin、protocol v8 模型和 report v2 probability exact binding，但当前仍只有测试模型。production 模型/authority key/policy/signer 未 provision 前，生产校准验收继续 incomplete。
...
- [ ] pair-local 多窗口 Top-K、编辑/删减分类 F1、双侧边界误差和同一片段多版本 many-to-many fine alignment 进入同一冻结 gold 证据链；已完成 gold native pair 唯一定位、完整 per-pair candidate inventory、selected fine window/TimeMap 摘要绑定以及 anchor、编辑、持续时间、source/target 四边界的唯一重算。显式 source/target 多版本组也已进入 native Evidence v5、fine frontier v3、formal receipt 和 acceptance：只有跨 pair 的共享对端与实际重叠区间才能证明 many-to-many，单 pair 多音轨成员不再被误算。当前获授权 frozen 编辑/多版本真实样本仍为 0，因此本项不能勾选，跨媒体关系 Top-K 也不得冒充这些指标。

当前剩余限制：逐段真实留出证据、双侧边界不确定范围、A/B 人工复核、blind runner、lifecycle Job、工程 raw v2、实际媒体卷/Job memory/终态清理回执、软件 live-process authority 和 fail-closed acceptance 虽已落地，但获授权且实际运行的真实冻结关系数仍为 0；modality 已与原生 pair-local evidence 闭环，校准概率的代码链也已绑定原生 margin 与批准模型，但没有获批真实 calibration split/model 时仍不得把工程分数当作可信概率。authority v3 challenge/replay、Authenticode artifact 和动态进程契约/CLI 已实现，但 production key/policy/signer/批准协议尚未配置，TPM/AIK 硬件增强、真实 calibration 模型和规定目标机正式运行仍未完成，20 套北极星长合集也未完成。formal 跨 tile 只证明完整候选关系排名，不能把 tile-local global assignment、fine TimeMap 或 same-segment many-to-many 当成一个单体超大 batch 的等价结果，因此不能据此宣称准确率或性能达标。
...
N×M blind 组件证据已进一步接入私有 formal matrix provenance v3。native batch schema v2 / Evidence v5 对全部 pair 输出 tile-local selection 与 fine frontier v3/selected execution 证据，并在 assignment/fine 前冻结固定版本的 intrinsic relation score；每个 score 同时绑定当前 native executable、固定 FFmpeg/FFprobe 与两端实际 spectral backend/fallback 的 execution identity，纯领域 contract 严格复核其 canonical digest、计数、rank、TimeMap identity、execution inventory、显式版本复用组与 source-major 顺序。formal plan 用 canonical physical candidate representative 把完整 query×candidate 矩阵拆成每侧最多 256、每 tile 最多 256 pair 的二维 tiles；协调器只做一次全 manifest preflight，顺序运行全部 tile，第一个 completed tile 固定 actual identity，任一失败或身份漂移都会丢弃 envelope。acceptance v6 保存完整 manifest、plan、execution suite、native receipt 与 raw，在检查无漏 cell、无 overlap、无参数漂移、无 execution identity 漂移、无 receipt/job replay 后，从全部 relation score 重算每个 query 的 global Top-K，并对 gold pair 的 fine/TimeMap 结果重算报告；protocol v8 / calibration report v2 再把 Top-1/Top-2 原生 score margin 与批准模型唯一绑定。私有 provenance 固定为 `releaseEligible:false` / `untrusted-self-consistent-provenance`。authority v3 已能固定 Authenticode artifact 和同一 live process 的终态 evidence inventory，但 production key/policy/signer 与真实获批 calibration 模型未完成；真实 frozen edit/many-to-many 样本也未闭合，跨 tile 排名仍不等于跨 tile fine/global assignment，所以这条接线不能冒充 release authority。

当前全局分配默认仍拒绝把同一物理内容区间同时映射到多个对端，被阻断的 pair 也不进入 fine；这适合一个长参考中互不重叠分集对应多个原片。只有用户显式把来源侧或目标侧的至少两个不同物理媒体声明为同一版本组时，coarse assignment 与 fine frontier 才允许组内共享相应物理区间；路径/FileId/hard-link 别名会失败关闭。pair-local evidence 只在同一 fine component 内观察到两个 completed selected pair 共享同一对端且重叠超过 tolerance 时报告 same-segment many-to-many，单 pair 的多音轨 `groupMemberRanks` 不再算作多对多。完整候选 inventory 和版本组已进入 formal receipt，但真实 frozen 多版本、编辑分类和双侧边界样本仍未运行，因此不能据此宣称长合集跨集错窗、删减分类或边界精度已经通过。
...
- [x] 独立 authority v3/challenge/expiry/replay ledger、P-256 签发 CLI、固定 signer Authenticode artifact attestation、同一 live-process 临时 Ed25519/OS 映像证明和 acceptance 验证器已闭环；默认无 production key/policy/signer 时仍失败关闭。
- [x] 可由独立信任根验证的同一 live-process 软件 challenge-response 已与 Authenticode artifact proof 闭环；TPM/AIK、硬件不可迁移密钥与受保护进程属于后续可选的更强威胁模型，不冒充当前软件证明。
