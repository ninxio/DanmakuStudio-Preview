...
当前项目文件 schema 为 v11。v7 引入多媒体库与 XML→参考素材绑定，v9 引入批量媒体候选，v10 引入四类分段 `MediaTimeMap`，v11 为 `verified` 增加绑定时间图核心摘要、revision、双端媒体身份和校准来源的验证记录。打开 v1-v10 项目会确定性迁移；旧时间规则只会成为 `legacy-unverified/blocked`，没有可信来源的旧 `verified` 会降为 `review`。保存前会再次做完整 schema 与反向引用校验。
...
- 当前 V2 已有 PTS、多音轨、声谱 landmark、速度漂移、双向编辑、局部边界和独立视觉回退的工程实现，但真实冻结集样本数仍为 0，尚未完成概率校准。自动结果最高为 `review`；正式按原片分集导出只接受带可信验证来源的 `verified` 时间图。因此本阶段 release 是安全预览，不代表准确率验收完成。
- 视觉回退目前只建立粗粒度全局仿射关系，不负责宣称局部删减边界；大幅裁切、极端调色、长静态或黑场会保守阻断。完整人工 A/B 同步播放和可持久验证签发仍待后续闭环。
...
- 完成可持久的人工 A/B 复核签发和双源同步播放；当前双时间轴分段面板只负责可解释定位，不冒充已播放复核。
