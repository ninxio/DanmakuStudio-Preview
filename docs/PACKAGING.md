...
原生 N×M batch 另有完整 source-major receipt v3：除 source/target inventory、全部 pair、tile-local Top-10、shortlist 状态和 TimeMap 外，每个 pair 还在全局 assignment 与 fine 之前冻结固定版本的 intrinsic relation score，并绑定当前 native 可执行文件、固定 FFmpeg/FFprobe 和两端实际 spectral backend/fallback 的 path-free execution identity；selected pair 还绑定 fine frontier、实际双侧 decode window 和 proposal TimeMap digest。completed receipt 只能声明一个由全部 pair 共同重算出的 identity digest；tile 内发生 Auto 回退分歧也会失败关闭。该原始 receipt 仍含内容身份和流元数据，只供本机内部协调器使用，不是可直接分享的匿名报告。普通产品和单个 native tile 每侧最多 256 个媒体、每次最多 256 pair。formal blind matrix v2 可把更大的 query×candidate 关系检索确定性拆成多个 tile，对完整 manifest 只预检一次，收齐每个 cell 后从全部 native relation score 重算 global Top-K；第一个 completed tile 固定 actual execution identity，任何缺失、重叠、参数漂移、执行身份漂移、候选同物理文件异流、receipt/job 重放或失败 tile 都不会生成 provenance。单批公开 accuracy 编译器继续要求本地候选全集完整，partial candidate shard 只能交给 exhaustive matrix 聚合器。视觉关闭时仅视频流不同不能冒充新候选，正式视觉模式则禁止 `null/auto` 流。公开组件结果仍固定为 `releaseEligible:false` / `untrusted-self-consistent-evidence`，不单独授予同一 pair 内窗口 Top-K、编辑/删减分类或边界准确度。私有 acceptance v4 会把 frozen case 唯一定位到 formal provenance 的 gold native pair，并从 fine frontier、selected execution 的实际双侧 decode window 与 proposal TimeMap 唯一重算 matched anchor、45 分钟漂移、编辑分类、持续时间及 source/target 四边界误差；TimeMap report 必须逐 case exact equal，修改数字后重签报告也会失败关闭。当前 native receipt 仍只给 component 总 `inventoryCandidateCount` 和选择/遗漏证明中的部分 candidate IDs，未枚举每个 pair 的完整窗口候选全集，因此多窗口 inventory 和 same-segment many-to-many 正式检查继续 incomplete。
...
完整 release 验收另需严格的 `C137AcceptanceBundle` 和外部信任。bundle v4 / protocol v6 / relationship report v3 只接受 formal blind provenance v3 / plan v2、native evidence/receipt v3、固定 score contract、exhaustive matrix coverage 与“从全部 cell 重算 global Top-K”的聚合契约；report 的 scope、score、K 以及每条 `decisionId/provenanceRef/case/gold/rankedCandidateIds/verified` 都必须逐项等于 formal 重算。manifest/gold/media/parameters/provenance root 分别绑定 bundle、dataset approval、preflight、runner 与 prediction receipt，公开聚合报告、单 tile Top-K 或旧 schema 不能代替该私有 envelope。performance report v3 仍只接受 formal raw schema v2，并独立检查实际媒体卷、Job memory、terminal cleanup 与 native attestation。仓库外 P-256 authority 已能签发一次性 challenge、原子消费 append-only ledger 并签名最终 bundle；但当前仍没有正式 provision 的 production key/policy，native execution 也没有 Authenticode/TPM attestation，modality/calibration 来源未闭合。安装包默认不携带可自行签发的私钥，因此正式结果继续是 `incomplete-evidence`。
