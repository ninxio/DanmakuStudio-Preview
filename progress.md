- 2026-07-20：修复“界面允许人工接管导出、原生写入器却拒绝，因此点击导出后没有文件”的双层规则冲突；C137 总 Goal 继续 active。
  - 根因不是文件保存到了未知位置：导出页会接受“人工接管 · 可导出”的正式关系，但生成写入凭据时仍强制要求算法 `verified` 且不得含 `ambiguous`；即使越过前端，Rust 写入器仍执行同一旧门槛，并残留已经从投影层移除的 5 条/1% 未映射弹幕数量限制。真实失败发生在创建文件之前，因此配置的导出目录保持为空。
  - 导出按钮现在就是一次明确用户动作：对已经满足媒体身份、连续合法 TimeMap、完整逐段证据和全部差异段明确分类的人工接管关系，导出时自动取得本机安装级 HMAC 凭据，不再要求用户寻找或重复点击第二个签发按钮；凭据仍绑定当前 map、revision、完整核心、两端媒体身份和人工接管时间。
  - Rust 写入器只对已签核心中的合法人工接管模式放宽算法质量门槛：`ambiguous` 必须逐段具有同索引的 `replacement` 人工结论，否则仍拒绝。版本替换段内弹幕按产品规则安全舍弃，不会线性投影到不同版本画面；普通低质量图、未分类歧义、错误媒体身份、篡改核心或签名仍失败关闭。
  - 原生写入层完整移除未映射弹幕数量阈值及其无用统计。原 XML 长于当前已确认来源范围时，范围外弹幕不导出但不再阻止合法分集落盘；新增 10 条范围外弹幕的原生写入回归证明文件仍可创建。
  - 回归新增“低自动质量 + 人工接管 + 版本替换段”实际写出 XML，以及导出动作取得安装级凭据的覆盖。验证结果：源码审计、ESLint、TypeScript/Vite production build 通过；Vitest **93 files / 978 tests** 全绿、1 项真实媒体环境测试按设计跳过；Rust **444 passed / 24 ignored / 0 failed**，strict Clippy `-D warnings` 通过；Chromium 四页北极星 E2E **6/6** 通过。
  - 同时把 ESLint 的 Rust `target` 忽略改为递归规则，避免误扫此前生成在嵌套目录中的 Tauri 自动代码；没有删除或覆盖既有构建产物。
  - 本切片安装器：`src-tauri/target-c137-export-write-fix/release/bundle/nsis/Danmaku Timeline Studio_0.1.0_x64-setup.exe`，**4,333,099 bytes**，SHA-256 `611E81A3DB8AD9EE61B6E61C5B9CD91BC4E4F49A5565E2FBC9D53324532C09F5`；便携版：`src-tauri/target-c137-export-write-fix/release/danmaku_timeline_studio.exe`，**16,901,632 bytes**，SHA-256 `7540200CA6EDEC226DC835F921784493C0FE57EF6D9E56073C118EB25C5991EE`。使用独立目标目录，不覆盖可能正在运行的旧版。
  - 诚实限制：该修复保证已经明确接管的合法映射能真正写出文件，不证明自动对齐和删减边界本身已经达到独立 Gold 精度。其余 Beta、负关系、真实编辑事件和重复性能验收仍未完成，因此不能结束 C137。

- 2026-07-19：C137 完成“长 XML 未覆盖数量不再阻断导出”开发切片；移除不符合产品边界的 5 条/1% 数量闸门，C137 总 Goal 继续 active。
  - 产品语义纠正：原 XML 可以合法地长于当前准备导出的原片范围；不在任何已确认来源段内的弹幕继续保持只读、不会被误投影，也会计入“意外未覆盖弹幕”，但无论数量多少都只显示黄色提示，不再把整个导出状态改成“有阻断”。
  - 真正的数据完整性闸门没有移除：媒体内容身份变化、TimeMap 缺失或结构非法、未解决歧义、负投影时间、投影超过目标原片时长，以及未完成明确人工接管等仍会阻断。用户若确实需要保留超出范围的弹幕，应补充对应来源段，而不是让系统猜测投影目标。
  - 领域回归用 12 条弹幕中的 6 条大比例未覆盖验证 `readyWithWarnings`；界面回归直接复现 **6551 条**未覆盖，验证显示“可导出（有提示）”、不再出现“安全阈值”，并在桌面导出目录已配置时保持“导出全部分集 XML”按钮可用。
  - 自动验收：ESLint 通过；TypeScript/Vite production build 通过；Vitest 全量 **93 files / 977 tests** 全绿，1 项真实媒体环境测试按设计跳过；Chromium 四页北极星 E2E 串行 **6/6** 通过。首次沙箱内 Tauri 打包因 esbuild `spawn EPERM` 失败，允许构建子进程后 Rust release 与 NSIS 完整成功，不是源码失败。
  - 本切片安装器：`src-tauri/src-tauri/target-c137-unmapped-warning/release/bundle/nsis/Danmaku Timeline Studio_0.1.0_x64-setup.exe`，**4,333,274 bytes**，SHA-256 `527AD0096DFBFDC4967BC722AD28154D920DD9C60DC89DAD457E9B298ABCF8E5`；便携版：`src-tauri/src-tauri/target-c137-unmapped-warning/release/danmaku_timeline_studio.exe`，**16,903,168 bytes**，SHA-256 `D05BCEA0E8BA4443EE83A1E3C1A1C1BEB2C7464AFDC232845BC9FEA152439BE2`。使用独立目标目录，没有覆盖可能正在运行的旧版。
  - 诚实限制：本切片修复的是错误的导出控制策略，不提升自动对齐、删减边界或延迟估计精度；独立 Gold、其余 4 套 1×5 Beta、负关系、真实编辑事件与重复性能/取消恢复验收仍未完成，因此不能结束 C137。

- 2026-07-19：C137 完成“人工接管直接导出与四结论不设死路”开发切片；自动质量门槛不再重复否决用户明确选择，C137 总 Goal 继续 active。
  - 产品决策分层：自动质量门槛只决定“能否自动确认”；用户勾选错位风险并点击“采用系统建议并允许导出”后，系统会采用每段最高可能性分类，建立 `人工接管 · 可导出` 的正式关系。人工方案仍显示为人工接管，不伪装成“算法已验证”，原始质量原因、未验证区间和潜在错位诊断全部保留。
  - 移除不必要的安装级二次签发前置：人工接管本身就是用户的明确产品决策，不再要求随后点击一次本机签名按钮。导出投影与项目健康检查统一识别人工接管方案；它只把算法低置信降为警告，不绕过媒体身份、确认状态、TimeMap 连续/单调结构、逐段证据完整性、差异段明确分类或当前文件身份一致性。
  - 候选控制不再只有一条灰色死路：`参考多出 / 原片多出 / 版本替换 / 无法判断` 四种结论始终显示且可点击。与当前双轴形状兼容的结论直接保存；不兼容的结论会自动打开对应类型的“调整边界与结构”，要求用户给出合法双轴边界，而不是隐藏或禁用选项。
  - `review`、`blocked` 和具备完整媒体身份/合法分段的旧候选均可显示人工接管入口；谨慎用户仍可选择“保存关系供试听复核”，两条路径不再混为同一道门。真正缺图、错误状态、缺媒体身份、非法/空 TimeMap、缺 XML 绑定或媒体文件变化仍严格阻断。
  - 回归验证：ESLint 通过；TypeScript/Vite production build 通过；Vitest 全量 **93 files / 976 tests** 全绿，1 项真实媒体环境测试按设计跳过；Chromium 四页北极星 E2E 串行 **6/6** 通过。E2E 实际让 5 个候选全部完成风险确认并进入人工接管可导出状态；浏览器环境仅因未配置桌面导出文件夹而不执行文件写入，映射门控已显示“可导出（有提示）”。
  - 本切片安装器：`src-tauri/target-c137-export-takeover/release/bundle/nsis/Danmaku Timeline Studio_0.1.0_x64-setup.exe`，**4,334,238 bytes**，SHA-256 `30F72F6C4CEBF8B0E48C9FF14854F873DF9E583910D88AC648262E0C30039553`；便携版：`src-tauri/target-c137-export-takeover/release/danmaku_timeline_studio.exe`，**16,903,168 bytes**，SHA-256 `169BB825B44F99F5AD481FBB6F9935584FB4B008EC378C530D78F35F1239BFAC`。首次沙箱内打包因 esbuild `spawn EPERM` 失败，按既有受限构建权限在沙箱外重跑后完整成功。
  - 诚实限制：这项改进解决“已有候选却不给用户导出权”的产品控制缺陷，不提高算法本身的匹配准确率，也不证明 Dark 的删减边界已经正确。人工接管导出的 XML 仍需抽查开头、中段和结尾；独立 Gold、其余 4 套 1×5 Beta、负关系、真实编辑事件与重复性能/取消恢复验收仍未完成，因此不能结束 C137。

- 2026-07-19：C137 完成“TimeMap 人工结构编辑与算法门控解耦”开发切片；用户不再被算法给出的双轴形状和唯一可选分类锁死，C137 总 Goal 继续 active。
  - 匹配页每个候选分段新增渐进披露的“调整边界与结构”：四种结果类型（共同内容、参考多出、原片多出、版本替换）始终可选，并可在同一原子操作中修改参考/原片的开始与结束边界。保存会同步前后相邻段的共享边界，结构不合法时给出具体原因，不再用灰色按钮代替可操作修正。
  - 新增真正的双轴拆分与合并：用户可分别输入参考拆分点和原片拆分点；零长度轴保持边界点、正长度轴必须位于段内。相邻同类段可合并，异类段不会被静默抹平成一条错误映射。快捷人工判定仍按当前形状显示最可能选项，系统建议继续自动采用，普通用户无需逐段点击唯一答案。
  - 所有人工结构修改都保持非破坏性：原始 XML 不变；TimeMap revision 增加，旧安装级签名、旧逐段分类、旧 A/B 播放证据和旧人工接管记录全部失效，算法诊断仍保留。修改后的候选范围、proposal TimeMap 与 matchRange 同步更新，可重新 A/B 复核，也可明确接受当前风险后再次建立并签发可导出方案。
  - 领域回归新增双轴边界传播、候选范围同步、拆分、合并、异类合并拒绝、非法形状拒绝和旧证据失效；组件/E2E 新增四种结构类型实际可达和折叠区键盘/可访问性路径验证。
  - 自动验收：ESLint 通过；TypeScript/Vite production build 通过；Vitest 全量 **93 files / 975 tests** 全绿，1 项真实媒体环境测试按设计跳过；Chromium 四页北极星 E2E **6/6** 通过。首次独立 Tauri 打包因沙箱禁止 esbuild 子进程报 `spawn EPERM`，按既有构建权限在沙箱外重跑后 Rust release 与 NSIS 完整成功，不是源码失败。
  - 本切片安装器：`src-tauri/target-c137-manual-structure/release/bundle/nsis/Danmaku Timeline Studio_0.1.0_x64-setup.exe`，**4,334,270 bytes**，SHA-256 `47936D80C55F9C60A976AA4359E580D7DFB55E014D497981D8DCCC1178E1337B`；便携版：`src-tauri/target-c137-manual-structure/release/danmaku_timeline_studio.exe`，**16,903,168 bytes**，SHA-256 `A6C39D4269AFB08FFA83E4B66B5778E94EFEBD11E772D2CC80FEA561B82A3FA3`。使用独立目标目录，没有强行关闭或覆盖正在运行的旧版。
  - 诚实限制：本切片解决的是“算法已经给出可用关系但产品不给用户修正/导出通路”的控制权缺陷，不证明当前自动边界已经达到 Gold 准确率。桌面窗口观察接口本轮返回 Windows 不支持错误，因此没有伪称完成新安装包的人工桌面截图走查；自动北极星走查已通过。其余 4 套真实 1×5 Beta、10 个负关系、30 个真实编辑事件、独立 Gold、重复性能与取消恢复验收仍未完成，不能结束 C137。

- 2026-07-19：C137 完成“自动门槛改为可接管状态、Dark 1×5 真实 XML 投影与跨集统计闭环”开发切片；已生成 5 份开发验收 XML 和新 release，M3 与 C137 总 Goal 继续 active。
  - 匹配页不再把结构有效但自动质量未达标的候选显示为红色死路：候选与质量徽标改为黄色“可人工接管”，导出状态明确显示“可建立人工方案”，主操作改为“采用系统建议，建立可导出方案”。用户接受潜在错位风险后，系统自动采用每段最高可能性分类；分段界面只展示与当前双轴形状兼容的选项，不再陈列不可用灰按钮。签发前仍保留全部算法诊断，真正缺图、身份/结构错误或无 XML 绑定仍不可导出。
  - 真实 Dark 1×5 v23 强制 RTX 4090/CUDA 热运行耗时 **475,245ms（7.92 分钟）**，5/5 pair 完成、运行失败 0、无 CPU 回退；E01–E05 coverage 为 **0.87 / 0.98 / 0.96 / 0.90 / 0.91**，P95 残差为 **266 / 11 / 149 / 200 / 19ms**。全部候选因严格自动标准保留为可复核 TimeMap，而不是丢失关系。回执为 `artifacts/c137-real-media-smoke-dark-1x5-v23-release.json`。
  - 新增环境门控的真实北极星导出验收：从原生批次回执建立 1×5 项目，走生产领域的系统建议接管、签发、投影、XML 序列化和重新解析链；逐条验证文本与除时间外的原始 `p` 字段保持、时间不为负且不超过目标时长，并验证跨集重复为 0。开发 XML 位于 `artifacts/c137-dark-m3-v1/`。
  - 修复跨集重叠记账缺陷：同一参考弹幕在 E04 的版本差异区间被解释为舍弃、随后又在 E05 成功映射时，旧逻辑仍把它计入 source-only。现在映射状态在全项目范围优先，104 条重复记账被消除；8614 条原始弹幕严格对账为 **8059 条导出 + 549 条明确参考独有/版本替换舍弃 + 6 条意外未覆盖**，合计一致，5 份 XML 条数依次为 **1702 / 1535 / 1406 / 1620 / 1796**。
  - 回归验证：源码审计通过，ESLint 通过；Vitest 全量 **93 files / 972 tests** 全绿，真实 Dark 环境验收另行 **1/1** 通过；TypeScript/Vite production build 通过；Chromium 四页北极星 E2E **6/6** 通过；Tauri release 与 NSIS 打包成功。应用内浏览器空项目四页动线加载正常且无 console warning/error；浏览器不能替代桌面本机签名。
  - release 安装包：`src-tauri/target/release/bundle/nsis/Danmaku Timeline Studio_0.1.0_x64-setup.exe`，**4,331,236 bytes**，SHA-256 `EF659AFAB8CF117A3D1C80B223EB4FAF6FA7151B53DFD71A8B0B485C5FC239E6`；便携版：`src-tauri/target/release/danmaku_timeline_studio.exe`，**16,899,072 bytes**，SHA-256 `27B299C4952DA82C507B1BBBE451AF6A7FB83687181C74086BC405DE5DB1DEAD`。
  - 诚实边界：这 5 份 XML 是开发验收产物，签发环节使用测试隔离的本地 authority，不是桌面安装级签名收据；尚无独立 Gold 证明最终同步 p95 ≤200ms，E01 内部留出 P95 仍为 266ms。还缺其余 4 套真实 1×5 Beta、10 个真实负关系、30 个真实编辑事件、规定次数的冷/热性能与取消恢复验收，因此不能将 M3 或 C137 标记完成。
...
- 2026-07-19：C137 完成“1×5 强制逐 pair fine、失败候选保留与人工恢复闭环”开发切片；M1 达成，M2/M3 与 C137 总 Goal 继续 active。
  ...
  - 真实 Dark 1×5 v20 强制 RTX 4090/CUDA 批次耗时 **1,080,135ms（18.00 分钟，3.60 分钟/集均摊）**，5/5 pair 完成、运行失败 0、无 CPU 回退，达到 25 分钟冷批次门槛。E01–E05 coverage 为 **0.467752 / 0.982116 / 0.964980 / blocked / 0.911445**；span 为 **86 / 12 / 34 / 0 / 18**，≤5 秒短编辑为 **37 / 5 / 14 / 0 / 6**。E02/E05 明显改善，但 E01 仍不可用、E03 P95 仍为 **44,950ms**、E04 因与 E03 的约 20 秒粗物理区间重叠而阻断，因此 M2 仍未达到。回执为 `artifacts/c137-real-media-smoke-dark-1x5-piecewise-anchor-v20.json`。
  - 候选生成缺陷已修复为 feature v21：exact-landmark 与抗转码近似声谱候选不再互斥；即使 exact 先返回弱候选，近似候选仍完整进入同一 temporal-window grouping 与 fine frontier，资源上限和确定性排序保持不变。真实 E01 找回 `offset=+22,500ms / scale=1.0` 的正确关系，候选数 **1→2**、coverage **0.467752→0.867598**、span **86→40**、短编辑 **37→15**；但可信留出 P50/P95/P99/max 仍为 **244/1,731/5,747/5,747ms**，冷运行 **670,083ms（11.17 分钟）**，故只可人工复核，不能自动导出。证据为 `artifacts/c137-real-media-smoke-e01-dual-coarse-v21.json`。
  - 下一结构问题已锁定：E03 的 27 个可信留出中存在 `+45s/+65.851s` 的重复内容碰撞，不能继续用单一粗仿射半径把它们全部当作时间金标准；E04 需要在保持项目级非重叠安全门的前提下，让 fine 后的实际内容占用参与候选恢复，而不是允许单 pair 人工签名绕过冲突。不会通过降低 P95、边界或物理占用门槛结束 C137。
  - 本切片自动验证：Rust 全量 **440 passed / 24 ignored / 0 failed**，Rustfmt 与 strict Clippy `-D warnings` 通过；完整 `corepack pnpm verify` 通过源码审计、ESLint、Vitest **93 files / 968 tests** 和 TypeScript/Vite production build。普通沙箱首次仍因 esbuild `spawn EPERM` 无法启动，允许同一验证命令启动子进程后全绿，不是测试失败。
  - 本切片 release 安装包：`src-tauri/target/release/bundle/nsis/Danmaku Timeline Studio_0.1.0_x64-setup.exe`，**4,321,586 bytes**，SHA-256 `FBADC42A35524128EB3208C812A356F68601E5FFEB4359F5A94625251C8CBD6F`；便携版：`src-tauri/target/release/danmaku_timeline_studio.exe`，**16,872,960 bytes**，SHA-256 `1247B78A75A7A4C369CCC7238529C0D74CB49A68CBB1C3CE789CDB0043AF7A6D`。
...
- 2026-07-19：根据用户真实 1×5 验收失败，建立 `docs/goals/C137-execution-plan.md`，将 C137 从宽泛技术清单收敛为 A–G 七个可退出阶段，并设置真实媒体、准确率、删减边界、最终 XML、RTX 4090 性能、UX、可靠性和 release 的硬结束指标。
  ...
  - 核心门槛包括 10 套北极星全部 5/5、跨集错配为 0、matched 投影误差 p95 ≤200ms/p99 ≤500ms、编辑事件 F1 ≥0.95、边界 p95 ≤300ms、最终 XML Gold 时间误差 p95 ≤200ms，以及 RTX 4090 单 pair 冷/热 p95 ≤10/5 分钟。
  - TPM/AIK、企业级 production authority 和公共发行代码签名从 C137 算法完成前置条件中分离；它们可继续作为发布治理增强，但不能掩盖或阻止本地产品真实可用闭环。
  - 本切片仅建立计划和结束定义，没有改动运行代码，因此未重新打包；执行从阶段 A 的真实 batch/frontier 回归开始。
...
- 2026-07-18：C137 完成第六阶段第三十一个切片“真实媒体留出锚点收敛、局部时间独立共识与微编辑抖动归一化”，完整验证并在独立目录重新打包；Goal 继续进行。
  ...
  - 可解释诊断：每次运行最多保留 32 条逐锚点诊断，包含双端时间、粗残差、最终残差或未映射状态和命中 span；从而能直接定位第一个偏移扩大的区域，而不是只看到总体 P95。
  - 抖动归一化：对 10 秒以内、至少 4 次交替、单次不超过 750ms 且净时长仍符合粗尺度的 SourceOnly/TargetOnly 锯齿执行平衡归一化；真实单处或两处微编辑不会被吞并。
  - 真实 RTX 4090 E01：强制 CUDA/cuFFT 的完整生产调用链耗时 **295,030ms**，留出锚点误差由旧实现的约 `P50 293,397ms / P95 2,008,634ms` 收敛至 **P50 50ms / P95 200ms / P99 200ms / max 200ms**；脱敏回执为 `artifacts/c137-real-media-smoke-e01-gpu-balanced-micro-edit-v1.json`，SHA-256 `8236DA696227B9402DD6E9FC1DFFBCB2387C70307B051B373102238CFD11D885`。
  - 仍然失败关闭：当前 11 个可信留出锚点仅 6 个可映射，42 个训练锚点中 3 个未映射；最大未验证间隙 496,350ms 超过 486,085ms 限制，另有 70 个 blocked spans、7 个 ambiguous spans 和 53 个歧义边界。因此提案仍不发布 TimeMap，不能宣称 C137 已完成。
  - 短媒体回归：不超过 60 秒的素材使用时长一半作为未验证间隙上限，长媒体仍维持时长五分之一门槛；16 秒 FFmpeg 集成样例恢复通过，长片质量门没有放宽。
  - 自动验收：Rust 全量 **419 passed / 23 ignored / 0 failed**；完整 `corepack pnpm verify` 通过源码审计、ESLint、Vitest **93 files / 964 tests** 和 TypeScript/Vite production build；Chromium 四页北极星 E2E **6/6**。
  - release 安装包：`src-tauri/target-c137-real-heldout-v1/release/bundle/nsis/Danmaku Timeline Studio_0.1.0_x64-setup.exe`，4,258,532 bytes，SHA-256 `9E281230BF541DDE7F26F62E23A109428CC012CC0108E221541EE9FDCB3AA3DE`；便携版：`src-tauri/target-c137-real-heldout-v1/release/danmaku_timeline_studio.exe`，16,661,504 bytes，SHA-256 `27AC71FA22194E156BCDE96E8D6ABE6B92E7D02DA2BC49592AE408A3911961DF`。使用独立目标目录，不覆盖可能仍在运行的旧版程序。
  - 本阶段 checkpoint 标签：`checkpoint/c137-real-heldout-p95-200-v1-20260718`。
...
- 2026-07-17：C137 完成第六阶段第二十九个切片“显式多版本复用、真实跨 pair many-to-many 证据与单次 CPU/GPU 选择”，完整验证并在独立目录重新打包；Goal 继续进行。
  ...
  - many-to-many 证据修正：不再把单个 pair 的多音轨 `groupMemberRanks` 冒充多对多。只有同一显式版本组中的两个已完成 selected pair，在同一 fine component 内共享同一对端、物理区间实际重叠超过 tolerance，才生成跨 pair 版本复用证据；修改组、pair、候选或窗口后不能通过 acceptance exact binding。
  - 产品入口：匹配页每次运行可直接选择“GPU（CUDA，不回退 CPU）”“CPU（禁用 CUDA）”或“自动（GPU 优先，失败回退 CPU）”，无需重新打开设置；高级区新增来源侧/原片侧“同一内容的多个版本”开关，默认关闭，选择不足两个素材时自动复位。1×N 长参考分集仍可保持默认互斥，多版本复用必须由用户明确开启。
  - 自动验收：完整 `corepack pnpm verify` 通过源码审计、ESLint、Vitest **93 files / 964 tests** 和 TypeScript/Vite production build；Rust 串行全量 **406 passed / 19 ignored / 0 failed**，`cargo fmt --check` 与 strict Clippy `-D warnings` 通过；Chromium 四页北极星 E2E **6/6**。E2E 首轮准确发现旧 mock 仍是 Evidence v4/schema v1，升级为 v5/schema v2 并补齐复用 inventory 后全绿；`git diff --check` 通过。
  - release 安装包：`src-tauri/target-c137-version-reuse-v5/release/bundle/nsis/Danmaku Timeline Studio_0.1.0_x64-setup.exe`，4,213,394 bytes，SHA-256 `26CB95278437E99C2C6DDC589F0D1777B9AEF8763D26DF9A5A0571BBAEDC812D`；便携版：`src-tauri/target-c137-version-reuse-v5/release/danmaku_timeline_studio.exe`，16,499,200 bytes，SHA-256 `8C248A5AB5CE2D6598AA0F42BA421CD104F45662E63370D72D0C3BED2165FAF6`。使用独立目标目录，不覆盖仍可能运行的旧版程序。
  - 诚实限制：本切片证明了多版本语义、执行合同和防伪证据链，不等于真实影片精度已经达标。当前获授权 frozen Gold、真实 calibration split/model 和真实 same-segment many-to-many 冻结观察仍为 0；production authority key/policy/signer、规定 4 核正式测量与 20 套真实北极星长合集也未完成，因此 C137 保持 active。
  - 本阶段 checkpoint 标签：`checkpoint/c137-version-reuse-v1-20260717`。

- 2026-07-17：C137 完成第六阶段第二十八个切片“同一原生进程的动态 challenge-response 与 Windows 映像证明”，完整验证并在独立目录重新打包；Goal 继续进行。
  - 原生进程证明：Tauri 后端新增进程内临时 Ed25519 会话，opening 绑定外部 challenge/256-bit nonce、PID、Windows 进程创建 FILETIME、当前 EXE 全文件 SHA-256 与临时公钥。formal blind batch 和 performance session 在真实启动时进入会话 inventory，只有原生任务进入成功/失败/取消终态后才能分别封存一次 `nativeJobId/receiptDigest` 或 `collector sessionId/evidenceDigest`；漏封、终态前封存、重复封存、跨 session 和 finalize 重放全部失败关闭。
  - authority v3：finalization 对 opening、排序后的完整终态 inventory 和动态 evidence root 做同一 Ed25519 签名。仓库外 CLI 在消费 challenge 前通过 Windows 独立读取 live PID 的启动 FILETIME 和映像路径，重新哈希进程映像并复核 Authenticode signer；最终 P-256 attestation 同时绑定 OS 进程观察、live-process receipt、完整 acceptance bundle、formal provenance、performance raw 和 append-only ledger。跨进程改写即使重签外层 authority 仍无法伪造内部 Ed25519；旧 authority v2、未知字段、进程/映像/evidence 漂移和重放均拒绝。
  - acceptance 门禁：有效 authority v3 现在可以解除 `native-blind-native-attestation` 和性能 `native-attestation`；`native-blind-ranking-provenance` 继续只由 formal exhaustive matrix 自身闭合。当前实现是软件 live-process attestation，不冒充 TPM/AIK、硬件不可迁移密钥、受保护进程或代码完整性证明。
  - 自动验收：Rust 串行全量 **403 passed / 19 ignored / 0 failed**，进程证明定向 **2/2**；strict Clippy `-D warnings` 与 Rustfmt 通过。完整 `corepack pnpm verify` 通过源码审计、ESLint、Vitest **93 files / 957 tests** 和 TypeScript/Vite production build；随后新增 authority v2 降级拒绝回归并定向 **8/8** 通过。Chromium 四页北极星 E2E **6/6**，`git diff --check` 通过。
  - release 安装包：`src-tauri/target-c137-live-process-attestation/release/bundle/nsis/Danmaku Timeline Studio_0.1.0_x64-setup.exe`，4,215,503 bytes，SHA-256 `578904E6B55ADFD96B71D13042E49BCE6DB5FFBB2C16E5118AA91C5272F02CB8`；便携版：`src-tauri/target-c137-live-process-attestation/release/danmaku_timeline_studio.exe`，16,484,352 bytes，SHA-256 `E32589FDCF97CE73FE48F2A42080AB7C40A35271A0E26AC368C79FEF54E8A67B`。使用独立目标目录，没有覆盖仍可能运行的旧版程序。
  - 诚实限制：当前 release 未签名，也没有 provision production authority key/policy/checkpoint/signer 白名单；获授权 frozen Gold 和真实 calibration split/model 仍为 0，规定 4 核正式测量、20 套真实北极星长合集及 same-segment many-to-many 冻结证据尚未完成。因此工程链不能自我晋级，C137 保持 active。
  - 本阶段 checkpoint 标签：`checkpoint/c137-live-process-attestation-v1-20260717`。

- 2026-07-17：C137 完成第六阶段第二十七个切片“原生关系分数的版本化校准模型与报告防篡改绑定”，完整验证并在独立目录重新打包；Goal 继续进行。
  - formal exhaustive-matrix 的派生决策现在除 global Top-K 外，还从同一完整 candidate universe 唯一发布 Top-1/Top-2 原生 relation score，并以固定 `normalized-top1-relation-score-margin-v1` 公式计算 0..1 margin；分数顺序、稳定破同分和 provenance observation digest 仍由全部 native matrix cell 决定，调用方不能另选局部 shard 或事后换分数。
  - acceptance 升级为 bundle v6 / protocol v8，calibration report 升级为 v2。protocol 内的 calibration model 必须绑定独立 `calibration` split、数据集版本/摘要、固定原生 score contract、2..64 个控制点和模型内容摘要；控制点必须从 margin 0 覆盖到 1、横轴严格递增、probability 单调不减，使用唯一的分段线性插值。pending approval 不能夹带模型或 approvalId，旧 protocol v7/report v1 严格拒绝。
  - `native-blind-calibration-provenance` 不再固定 incomplete：每个 frozen decision 的 `decisionId`、Top-1/Top-2 score、规范化 margin、模型摘要、插值 probability 和由 Top-1/gold 重算的 correct 必须逐项等于 formal provenance 与批准模型。修改任一分数、margin、probability 或 model digest 后，即使重算 report evidence digest，也会失败关闭；ECE/Brier 只在这条原生绑定之外继续计算统计门槛。
  - 自动验收：定向 formal provenance **19/19**、acceptance **66/66**；完整 `verify` 通过源码审计、ESLint、Vitest **92 files / 952 tests** 和 TypeScript/Vite production build；Chromium 四页北极星 **6/6**。Rustfmt 与 strict Clippy `-D warnings` 通过。Rust 首轮 **400 passed / 1 failed / 19 ignored**，唯一失败是既有进程监督取消测试在全量负载下先返回 Timeout；该项立即单测 **1/1** 通过，降低负载后完整第二轮 **401 passed / 19 ignored / 0 failed**，未修改或放宽原生测试。
  - release 安装包：`src-tauri/target-c137-calibration-provenance/release/bundle/nsis/Danmaku Timeline Studio_0.1.0_x64-setup.exe`，4,193,349 bytes，SHA-256 `2CBB41E4392B90C26F0B3919E4CEB865A42A6508BC19EECF0922C31123CADCED`；便携版：`src-tauri/target-c137-calibration-provenance/release/danmaku_timeline_studio.exe`，16,346,624 bytes，SHA-256 `2D9BE9AA69BF5E2A7276040064DAAD2A59F44D67B8FF42F4B8A03470E53775EB`。旧版 08:53 进程仍可能有未保存项目，本轮没有强行关闭，而是使用独立目标目录。
  - 诚实限制：本切片完成的是“怎样从原生关系分数唯一重算受批准模型约束的 probability”代码链，不代表已经拥有可信生产概率。当前获授权 frozen Gold 和真实 calibration split 仍为 0，协议中的实际 production 模型/审批/authority policy 尚未 provision；live-process/TPM 证明、规定 4 核正式测量和 20 套真实北极星长合集也未完成，因此 C137 保持 active。
  - 本阶段 checkpoint 标签：`checkpoint/c137-native-calibration-v1-20260717`。

- 2026-07-17：C137 完成第六阶段第二十六个切片“native pair-local modality 来源与冻结报告绑定”，完整验证并在独立目录重新打包；Goal 继续进行。
  - pair-local fine evidence 升级为 v2：每个 frozen gold pair 现在从同一 native `proposalTimeMap` 唯一推导 `same-audio` / `visual-only` / `mixed` modality，并绑定音频/视觉 evidence types、实际 anchor 数、双端音视频流索引、fine score 和 proposal 摘要。audio/visual type 与 anchor 数不一致、音轨未等于 fine execution 实际流、视觉有锚点但缺双端视频流、TimeMap 已 blocked 或成功候选没有任何可用模态时全部失败关闭；不会把无证据候选事后包装成 `no-common-content`。
  - acceptance 的 `native-blind-modality-provenance` 不再是固定 incomplete：只有 relationship report 的每个 case 唯一、无重复并逐项等于 native pair-local 推导时才通过。调用方把 `same-audio` 改成 `visual-only` 后，即使重算 report evidence digest，仍会被 exact binding 拒绝。校准概率没有被偷换：当前 fine/relationship 分数仍是未校准 native score，`native-blind-calibration-provenance` 继续 incomplete，下一步需用获批 calibration split/model 把 native score 唯一映射成 probability。
  - 自动验收：针对性 **2 files / 61 tests** 通过；完整 `verify` 通过源码审计、ESLint、Vitest **92 files / 943 tests** 和 TypeScript/Vite production build；Chromium 四页北极星 **6/6**。普通沙箱首次因 Windows `spawn EPERM` 无法启动 esbuild，允许项目验证子进程后同一完整命令全绿。Rust 全量 **401 passed / 19 ignored / 0 failed**，`cargo fmt --check` 与 strict Clippy `-D warnings` 通过。
  - release 安装包：`src-tauri/target-c137-modality-provenance/release/bundle/nsis/Danmaku Timeline Studio_0.1.0_x64-setup.exe`，4,196,191 bytes，SHA-256 `4382BE784A973DABFD6747CCF2ACFE3C9AFC3D4C93C4ECD9E6137435CA0849DA`；便携版：`src-tauri/target-c137-modality-provenance/release/danmaku_timeline_studio.exe`，16,346,624 bytes，SHA-256 `1AD02A6DAD0DD61442E232428A8C91B737492B58A522545A11B60F3C51A89890`。
  - 诚实限制：本切片关闭的是 modality 报告可被调用方改写的证据断点，不代表真实准确率已通过。获授权 frozen Gold 仍为 0；native score 的获批校准模型、production authority key/policy/signer、live-process/TPM 证明、规定 4 核正式测量和 20 套真实北极星长合集仍未完成，因此 C137 保持 active。
  - 本阶段 checkpoint 标签：`checkpoint/c137-native-modality-v2-20260717`。

- 2026-07-17：C137 完成第六阶段第二十五个切片“外部 Authenticode artifact attestation 与动态进程边界”，完整验证并在独立目录重新打包；Goal 继续进行。
  - 仓库外 authority 契约升级为 v2：out-of-band policy 除 P-256 authority 公钥/checkpoint 外，必须固定 1–32 个 canonical Authenticode signer 叶证书 SHA-256，并强制时间戳证书。`inspect-native`、`attest`、`verify` 会对指定 EXE 流式计算前后两次全文件 SHA-256、检查期间大小/内容不变、调用 Windows `Get-AuthenticodeSignature` 要求 `Valid`，再绑定 signer/timestamp 证书摘要、EXE 大小、inspection time 和完整 artifact attestation digest。
  - post-run authority 签名同时绑定完整 acceptance bundle、formal provenance、performance raw、全部 native pair 唯一 executable digest 与 artifact attestation；`verify` 必须重新提供 EXE 并独立复核稳定身份。换 EXE、换 signer 白名单、缺时间戳、未签名/签名损坏、摘要不等于 formal execution identity、旧 authority v1 schema、proof/binding/ledger 任一篡改均失败关闭。PowerShell 路径通过环境变量传入，不拼接用户路径；优先使用可用的 PowerShell 7，Windows PowerShell fallback 固定系统 Security module 路径。
  - 信任边界没有偷换：新增 blind/performance 两个 Authenticode artifact 检查，authority v2 可将其解除；`native-blind-native-attestation`、`native-blind-ranking-provenance` 和性能 `native-attestation` 仍保持 incomplete。签名磁盘 EXE 与 evidence identity 一致，不等于证明动态 job/receipt 确由同一个 live process 产生；下一步仍需同进程 challenge-response + OS 进程映像复核或 TPM/AIK。
  - 自动验收：源码审计和 ESLint 通过；Vitest **92 files / 941 tests** 全绿，其中 authority CLI 实际使用已安装且 Authenticode 有效的 `node.exe` 完成 init → issue → attest → verify → replay 拒绝闭环，并验证未签名假 EXE 失败；TypeScript/Vite production build 通过；Chromium 四页北极星 **6/6**。Rust **401 passed / 19 ignored / 0 failed**，`cargo fmt --check` 与 strict Clippy `-D warnings` 通过。完整 `verify` 首次在 941 项测试全绿后发现测试变量缺少显式模板字符串类型，补成 `C137Digest` 后 lint/build 复跑全绿。
  - release 安装包：`src-tauri/target-c137-authenticode-artifact/release/bundle/nsis/Danmaku Timeline Studio_0.1.0_x64-setup.exe`，4,195,349 bytes，SHA-256 `A9186BD2D4C6B40A47F6A6DF272F6C3CC7C1AB844D19960A9F68F3FE5D634E15`；便携版：`src-tauri/target-c137-authenticode-artifact/release/danmaku_timeline_studio.exe`，16,346,624 bytes，SHA-256 `5FAA2C865DBFE7D8F39A23E8FA396BA4D0489DDD0A70C08315DD82F5DBA3E881`。
  - 当前 release 仍未签名；对新便携版执行真实 `inspect-native` 返回 `NotSigned` 并拒绝生成 attestation，证明工程包不能自我晋级。正式代码签名证书、production authority key/policy/checkpoint、live-process/TPM 证明、modality/calibration provenance、获授权 frozen Gold、规定 4 核正式测量与 20 套真实北极星长合集仍未完成，因此 C137 保持 active。
  - 本阶段 checkpoint 标签：`checkpoint/c137-authenticode-artifact-v1-20260717`。

- 2026-07-17：C137 完成第六阶段第二十四个切片“完整 per-pair fine candidate inventory 与验收绑定”，完整验证并在独立目录重新打包；Goal 继续进行。
  - 原生 fine frontier contract 升级为 v2，native Evidence/receipt 升级为 v4：每个 component receipt 现在完整发布 path-free `inventoryCandidates`，逐项包含 canonical `pairOrdinal/candidateOrdinal`、coarse upper bound，以及组成该时间窗组的全部 relation members 和双轴内容区间；候选 ID 必须按 pair 连续从 1 枚举，不能再只靠 component 总 `inventoryCandidateCount` 猜测某个 pair 的窗口清单。
  - `inventoryDigest` 改为对完整清单做 domain-separated canonical SHA-256，外层 receipt digest 再绑定该清单与摘要。Rust、Tauri bridge 和 formal blind domain contract 均会独立复算；删除候选、改动窗口/成员、重复或断开 candidate ordinal，即使调用方重签外层 receipt 或同时重算摘要也会失败关闭。
  - pair-local frozen Gold 证据不再从 selected/runner-up/omitted 的部分 ID 推断：直接按 gold pair 过滤完整 inventory，发布精确 `pairCandidateCount`，并只在清单确实存在至少两个同 pair 候选时证明多窗口 alternative。C137 acceptance 升级为 bundle v5 / protocol v7，并严格要求 native evidence/receipt v4 与 fine contract v2；旧契约不能混入新验收语义。
  - 新增前后端篡改回归，并同步 formal fixture、真实 batch runner、匹配页测试和 Playwright 原生 mock。全量验证通过：source audit、ESLint、TypeScript/Vite production build，Vitest **92 files / 939 tests**，Playwright 四页北极星 **6/6**，Rust **401 passed / 19 ignored / 0 failed**，`cargo fmt --check` 与 strict Clippy `-D warnings`。普通沙箱两次因 Windows `spawn EPERM` 未能启动 esbuild，改为沙箱外受限并发后全量套件完整通过；第一次 180 秒工具时限留下的测试子进程已精确清理，没有终止用户原有 Node 进程。
  - release 安装包：`src-tauri/target-c137-fine-inventory/release/bundle/nsis/Danmaku Timeline Studio_0.1.0_x64-setup.exe`，4,192,354 bytes，SHA-256 `5AA3CEF8C369705860B9EFB41E6237BAA4F8C7651AD022FEE0CE01E928359718`；便携版：`src-tauri/target-c137-fine-inventory/release/danmaku_timeline_studio.exe`，16,346,624 bytes，SHA-256 `FC2C2068D9EF3F26BDC73575B3C601B6FEA8B606056F6EC882D057A85A569FBF`。
  - 诚实限制：本切片完成了完整候选库存的可复核证据链，不等于真实影片精度已经达标。当前获授权 frozen Gold/编辑/同 pair 多窗口/多成员样本仍为 0，正式 provision 的 production authority policy、独立 native Authenticode/TPM attestation、modality/calibration provenance、规定 4 核正式测量和 20 套真实北极星长合集仍未完成，因此 C137 保持 active。
  - 本阶段 checkpoint 标签：`checkpoint/c137-fine-inventory-v2-20260717`。

- 2026-07-17：C137 完成第六阶段第二十三个切片“pair-local frozen Gold 重算、TimeMap 报告绑定与双轴边界证据”，完整验证并在独立目录重新打包；Goal 继续进行。
  - 新增纯领域 `c137PairLocalFineEvidence`：每个 frozen case 以双端完整媒体身份/显式流 commitment 唯一定位 formal provenance 中的 gold native pair，绑定 batch/pair ordinal、resolved fine frontier receipt、selected candidate、实际 source/target decode window 与 proposal TimeMap digest；缺失、重复定位、frontier proof 不闭合或 anchor 未完全映射均进入 blocked。
  - 不再信任调用方填写 TimeMap 精度数字：从同一 frozen Gold 和 native proposal 唯一重算 matched anchor 绝对误差、45 分钟漂移、三类编辑事件、持续时间误差以及 sourceStart/sourceEnd/targetStart/targetEnd 四个双轴边界误差；acceptance 新增 pair-local integrity、TimeMap exact binding、完整窗口 inventory、same-segment many-to-many 与双轴边界五项检查。即使修改误差后重签 report evidence digest，exact binding 仍会失败关闭。
  - 证据层明确区分“component 总候选数”和“某 pair 的完整窗口候选清单”：现有 native receipt 只公开 `inventoryCandidateCount` 及选择/runner-up/omitted 中出现的部分 candidate IDs，因此 `completePairCandidateInventoryEnumerated` 固定为 false，不能把总数冒充 pair-local Top-K；selected temporal group 的 `groupMemberRanks` 可证明已选组内多成员，但没有真实 frozen 多成员样本时 many-to-many 检查仍不通过。
  - 新增 3 个 pair-local 单元测试，并扩展 acceptance 失败闭合测试；全量验证通过：source audit、ESLint、生产 Web build，Vitest 92 文件 / 938 项，Playwright 6/6，Rust 400 passed / 19 ignored / 0 failed，`cargo fmt --check` 与严格 Clippy。全量验证开始时曾因并行残留 cargo 造成两次 Windows `spawn EPERM`，清理并串行后完整通过；Tauri 重复前置 Web build 同样被系统拒绝启动 esbuild，因此先独立验证最新 `dist`，再用临时 merge config 仅跳过重复的 `beforeBuildCommand`，release Rust 编译和 NSIS 打包完整成功，临时配置已删除。
  - release 安装包：`src-tauri/target-c137-pair-local/release/bundle/nsis/Danmaku Timeline Studio_0.1.0_x64-setup.exe`，4,194,678 bytes，SHA-256 `E8C9041E4C197207304561626719914E77B09A1A35DD7C379A8A1F02CD4647EA`；便携版：`src-tauri/target-c137-pair-local/release/danmaku_timeline_studio.exe`，16,340,992 bytes，SHA-256 `19EC16A846ADB94F1D68214A16708DF92064E298C0E6B32850529F20EDE6A2E6`。
  - 诚实限制：本切片完成的是 pair-local Gold/TimeMap 证据的唯一派生和报告防篡改，不是准确率通过。native receipt 的完整 per-pair candidate inventory 仍未进入 schema，获授权真实 frozen Gold/编辑/多成员样本仍为 0；production authority key/policy、native Authenticode/TPM attestation、modality/calibration provenance、规定 4 核正式测量和 20 套真实北极星长合集也未完成，因此 C137 保持 active。

- 2026-07-17：C137 完成第六阶段第二十二个切片“仓库外 authority、一次性 challenge 与有状态防重放账本”，完整验证并在独立目录重新打包；Goal 继续进行。
  - 新增 `C137AuthorityProofV1` 加密证据链：运行前 challenge 用外部 ECDSA P-256 authority 私钥签名 256-bit nonce，并锁定 protocol、manifest、dataset/certification、blind/performance plan、环境、release build 与参数；运行后 attestation 再锁定完整 acceptance bundle、formal blind provenance、performance raw evidence 和全部 native pair 唯一一致的 executable digest。
  - append-only ledger 对每个 challenge 只允许一次 `issued` 和一次 `consumed`，连续 sequence、完整 actions digest、前一 checkpoint、外部 minimum sequence 与可选固定 checkpoint 共同阻断回滚；重复消费、先消费后签发、跨 bundle 复用、过期、伪密钥、公钥/keyId 不一致、任一 binding/签名/ledger 篡改均失败关闭。
  - 新增真正可操作的 `corepack pnpm c137:authority -- <init|issue|attest|verify>` 外部 CLI：私钥路径位于项目仓库内时直接拒绝，初始化拒绝覆盖既有 key/policy/ledger；attest 在同一个 authority ledger 中原子消费 challenge，第二次签发失败，proof 可由 out-of-band public policy 和最低账本序列独立复核。安装包不包含私钥，也不内置可自批的 production 公钥白名单。
  - acceptance 新增异步加密验证入口：只有 proof、trust policy、完整 bundle 与账本 checkpoint 全部验证通过，才解除 `external-trust-authority`、blind plan authority、challenge freshness 和 replay ledger 检查；`native-blind-native-attestation` 与性能 `native-attestation` 仍保持 incomplete。普通 P-256 authority 只能证明独立审批者签过这些字节，不能冒充 Authenticode/TPM 对实际进程的证明。
  - 自动验收：authority/CLI/acceptance 定向 **3 files / 62 tests** 通过；`corepack pnpm verify` 通过源码审计、ESLint、Vitest **91 files / 934 tests** 和 TypeScript/Vite production build，Chromium 四页北极星 **6/6**。Rust 串行全量 **400 passed / 19 ignored / 0 failed**，Rustfmt、strict Clippy `-D warnings` 与 `git diff --check` 通过。首次全量 lint 发现独立 Tauri `target-*` 自动生成 JS 被误扫，补齐 ESLint ignore 后相同完整验证全绿，不是业务失败。
  - 本阶段安装器：`src-tauri/target-c137-authority/release/bundle/nsis/Danmaku Timeline Studio_0.1.0_x64-setup.exe`，大小 `4193524` 字节，时间 `2026-07-17 10:10:29 +08:00`，SHA-256 `A75FE1D4B40CC5ADEE2C0727CB5E0EC577AFB26E633030CCDD89116794F4B1D1`。
  - 本阶段便携版：`src-tauri/target-c137-authority/release/danmaku_timeline_studio.exe`，大小 `16340992` 字节，时间 `2026-07-17 10:10:29 +08:00`，SHA-256 `FA844800A952767A5FBBCC5DDB2D25D41A88ADB0B7CCD43645DE922973E757E7`。旧版程序可能仍有未保存项目，本轮继续使用独立目标目录，没有强行关闭用户进程。
  - 诚实限制：本切片完成的是可独立持钥的 plan/challenge/replay authority 基础设施，不是 production trust root 实例；当前没有正式 provision 的私钥/公钥/checkpoint policy，也没有 Authenticode/TPM native execution attestation。获授权冻结真实 Gold 仍为 0；modality/calibration provenance、pair-local 多窗口/编辑/双边界 frozen evidence、4 核正式测量和 20 套真实长合集仍未完成，因此 C137 保持 active。
  - 本阶段 checkpoint 标签：`checkpoint/c137-external-authority-v1-20260717`。
...
- 2026-07-14：C137 完成第六阶段第十六个切片“最终 TimeMap 变速闭环、CPU/GPU 请求级策略与候选 lineage 加固”，完整验证并重新打包；Goal 继续进行。
  ...
  - 质量证据改为最终图权威：训练/真实留出 anchor 全部重新穿过冻结后的分段 TimeMap，落入 sourceOnly、ambiguous 或图外的 anchor 计为未映射失败，不再被静默丢弃；图级 coverage 使用最终 matched source 时长，P50/P95/P99/max 使用最终 held-out 重投影。blocked affine 图也只展示最终图的 0 coverage、未映射数和最终残差，不再回退漂亮的 coarse 数字；右端点插值与导出采用同一 `targetEnd - 1` clamp。
  - 候选与确认 lineage 失败关闭：proposal→candidate 使用覆盖媒体、流、identity、完整 spans、质量/evidence、engine/feature/parameters 的 semantic digest；candidate→confirmed 使用锁定映射与 provenance 的 immutable lineage。相同外层区间但内部 spans 被替换、confirmed clone 篡改或 ownerless confirmed route 均拒绝；v1-v12 ownerless legacy route 只通过稳定 legacy accepted owner 迁移，不静默删除用户来源段。
  - 设置中心新增“自动推荐 / 强制 GPU / 强制 CPU”三态“声谱计算策略”，保存后统一作用于单次、N×M 批量、精度 benchmark 与性能 benchmark。自动模式允许 CUDA 失败后从头回退 CPU；强制 GPU 的能力探测、初始化或执行失败会停止任务且不静默回退；强制 CPU 不探测或执行 CUDA。请求在启动 worker 前解析为不可变 snapshot，缓存键同时绑定 requested policy 与实际 backend；benchmark plan、case 参数摘要和性能 digest 同样绑定策略。
  - 设置与错误边界同步收紧：旧备份缺少字段时确定迁移为 `auto`，显式未知策略和 native `spectralBackendTypo` 直接拒绝；桌面配置解析失败不会覆盖浏览器 fallback。CUDA 错误只允许从字符串起点或受控包装前缀匹配白名单码并映射固定中文提示，任意嵌入、前后缀伪造和未知原生错误继续走通用脱敏。
  - RTX 4090 环境无需安装：再次实测 `NVIDIA GeForce RTX 4090`、24564 MiB、driver `610.74`、compute capability 8.9；显式注入 CUDA Toolkit 13.3 runtime 后强制 `C137_REQUIRE_CUDA_FFT=1` 的 CUDA/cuFFT 回归 **26/26** 实际通过、0 跳过。GPU 只加速 coarse/fine 声谱 FFT；FFmpeg 解码、全文件哈希、landmark pairing、edit-aware DP、边界精修与 exact assignment 仍使用 CPU。
  - 自动验收：修复后的 Rust 与 TypeScript 独立复查均无 P0/P1，审查提出的短 gap、blocked evidence、strict request 与错误白名单 P2 已修复。Rust 串行全量 **387 passed / 18 ignored / 0 failed**，Rustfmt 与 strict Clippy `-D warnings` 通过；最终 `corepack pnpm verify:release` 完整通过，包含源码审计、ESLint、Vitest **83 files / 864 tests**、TypeScript/Vite production build、Chromium 四页北极星 **6/6** 和 Tauri NSIS release。普通沙箱首轮因 esbuild `spawn EPERM` 不能启动，允许构建子进程后相同完整命令全绿，不是代码失败。
  - 本阶段安装器：`src-tauri/target/release/bundle/nsis/Danmaku Timeline Studio_0.1.0_x64-setup.exe`，大小 `4137474` 字节，时间 `2026-07-14 00:37:41 +08:00`，SHA-256 `8557029E173E37C30C70143A6C4987BA461B2D20FD8875E4B518F05E6CBC1671`。
  - 本阶段便携版：`src-tauri/target/release/danmaku_timeline_studio.exe`，大小 `16241664` 字节，时间 `2026-07-14 00:37:41 +08:00`，SHA-256 `2734934E9906C951F26CB8130DA0A025FB63F7142F86B6433EE8211801E13B5A`。
  - 诚实限制：本切片证明了合成变速/微编辑、最终证据闭环、策略隔离和真实 4090 执行，但获授权真实冻结关系仍为 0，尚不能证明用户实际影片上的整体匹配准确率、删减 F1 或双边界 P95；双人标注/校准、20 套真实北极星长合集与外部 trust root 仍未闭环，因此 C137 保持 active，不能开放自动 `verified`。
  - 本阶段 checkpoint 标签：`checkpoint/c137-speed-drift-compute-policy-v1-20260714`。
- 2026-07-13：C137 完成第六阶段第十五个切片“adaptive 多窗口 fine frontier 生产接线、二次 exact assignment、Evidence v3 与原生 UI 权威”，完整验证并重新打包；Goal 继续进行。
  - 多窗口 fine 已进入真实生产链：每个 physical-media component 保留受显式上限约束的完整 coarse candidate universe，先按双轴时间窗口形成 alternative group，再迭代精修组内全部 eligible member；omitted optimistic upper bound、runner-up 与 strict margin 共同决定 resolved / unresolved，证据不足不会伪造成低置信成功。最终依据 proposal TimeMap 的 source/target 整数毫秒 occupancy 再做一次 exact assignment，并重放校验 score、occupancy、TimeMap 与 execution digest，coarse globalSelection 只保留为诊断，不能覆盖 fine 最终选择。
  - 性能与资源边界同步收紧：component 级累计 round、fine member、wall-time 与 exact-search state/expansion/interval budget，单组最多执行 64 个 intrinsically eligible member，任何超限在下一次 expensive fine 前失败关闭。最终 winner 直接复用首次精修得到的 proposal/evidence，删除一次重复的 FFmpeg window decode、CUDA fine FFT、CPU edit-aware DP 与边界精修；resource/infrastructure failure 会使整个 component 原子 Failed、清除 partial TimeMap，EvidenceBlocked 则保持可审计 unresolved。取消仍在最终媒体/工具身份复核前丢弃全部 staged 结果。
  - Evidence v3 原子闭环：native evidence/receipt、blind runner 与 formal provenance 全部升级到 v3，acceptance bundle/protocol 升级到 v4/v6；fineFrontier 固定库存、状态计数、累计搜索、证明与最终选择，fineExecutionEvidence 固定实际音轨、requested/effective window、coarse/fine backend、参数、occupancy、TimeMap、score 与摘要。跨 Rust/TypeScript 的 domain-separated SHA-256 对 JSON number 使用 IEEE-754 binary64 十六进制 canonical form，并拒绝 JavaScript 非安全整数；旧 v2、混合版本、摘要篡改与重签 backendDetail 篡改全部失败关闭。
  - 原生结果成为匹配页唯一权威：前端不再自行运行 global assignment、按剧集顺序重排或补造 TimeMap；只有 fine receipt 已 resolved、pair 位于 selectedCandidateIds、bestCompleted 与 component/pair/execution/TimeMap digest 全部闭合时才写入候选。未选中的 completed alternative、unresolved、no-eligible、resource、infrastructure 与 cancelled 均不生成可确认候选；界面继续使用项目内已导入素材，不要求用户重新选择路径，并以“目标集 ← 来源时间段”展示多视频结果。
  - 当前多视频能力：一个原生任务继续支持总计最多 256 pair 的 N×M 评估；互不冲突的分集区间可形成 many-to-one、one-to-many 与 many-to-many 关系图，同一时间区间的多个剪辑/音轨版本会共同进入 pair-local fine competition，再由第二次 occupancy assignment 选择有证明的非冲突组合。相互重叠且同时占用同一 physical interval 的多个结果不会被重复发布，而是作为 alternative 保留或保持 unresolved。
  - RTX 4090 环境无需安装：实测 `NVIDIA GeForce RTX 4090`、24564 MiB、driver `610.74`、compute capability 8.9，CUDA Toolkit `13.3` / nvcc `13.3.73` 与 `cufft64_12.dll` 均已存在。显式注入 CUDA runtime 并设置 `C137_REQUIRE_CUDA_FFT=1` 的真实 CUDA/cuFFT 回归 **25/25** 通过，强制模式不能静默回退 CPU；coarse 与 fine 的 backend ID/requested/detail/fallback 四字段连续性已由 Rust 与 TypeScript 双边锁定。完全退出并重启应用后即可继承已持久配置的 PATH。
  - 自动验收：独立两轮 P0/P1 审查无剩余阻断。Rust 串行全量 **372 passed / 18 ignored / 0 failed**，frontier 定向 **9/9**，Rustfmt、strict Clippy `-D warnings` 与 `git diff --check` 通过。最终 `corepack pnpm verify:release` 完整通过：源码审计、ESLint、Vitest **82 files / 832 tests**、TypeScript/Vite production build、Chromium 四页北极星与 Evidence v3 **6/6**、Tauri NSIS release；普通沙箱首次因 esbuild `spawn EPERM` 不能启动，允许构建子进程后 E2E 暴露旧 CUDA mock detail，夹具按真实 lock 修复且生产校验未放宽，随后相同完整命令全绿。
  - 本阶段安装器：`src-tauri/target/release/bundle/nsis/Danmaku Timeline Studio_0.1.0_x64-setup.exe`，大小 `4124071` 字节，时间 `2026-07-13 22:50:37 +08:00`，SHA-256 `A155A05673F6247513592446119653E5E64F018511F3996691E6B0A908B66860`。
  - 本阶段便携版：`src-tauri/target/release/danmaku_timeline_studio.exe`，大小 `16167424` 字节，时间 `2026-07-13 22:50:37 +08:00`，SHA-256 `F6E00D6F6652DA694CA3A44A106EC2D800CB5BDCEC09B47D2B1077D161C4AB4F`。
  - 诚实限制：本切片证明了受限候选空间内的确定性选择、证据完整性、资源/取消原子性和真实 4090 执行，但尚不能证明用户真实影片上的整体准确率或所有删减双边界都已准确；获授权冻结 real gold、双人标注/校准、20 套真实北极星长合集与外部 trust root 仍未闭环，因此 C137 保持 active。4090 主要加速声谱 FFT；FFmpeg 解码、完整哈希、landmark pairing、edit-aware DP、边界精修与 exact assignment 仍使用 CPU，高 CPU 占用会下降但不会归零。
  - 本阶段 checkpoint 标签：`checkpoint/c137-adaptive-fine-evidence-v3-20260713`。
- 2026-07-13：C137 完成第六阶段第十四个切片“4090 有界 fine FFT、rolling-row edit DP、缓存感知联合资源事务与 fine frontier 核心”，完整验证并重新打包；Goal 继续进行。
  - 4090 已进入实际 fine 生产链：原生 feature 升级到 v13；coarse 实际在 CUDA 完成时，候选所需的 selected-window fine FFT 会锁定同一 CUDA/cuFFT 后端，按最多 4096 帧 × 512 点的有界批次执行，并立即归约为最终 14 维 fine feature，不保留随视频时长增长的声谱矩阵。Auto 只有在 coarse 实际回退 CPU/hybrid 时让 fine 保持 CPU；显式锁定 CUDA 的初始化或执行错误失败关闭，不会把部分 GPU 与 CPU 特征静默混用。CPU/CUDA 数值等价、批次 seam、取消与 fallback 均有回归。
  - edit-aware DP 降低 CPU 内存压力：三份完整 `i64` 成本面改为六条 rolling row，保留三份紧凑 `u8` parent 面以进行完全相同的 traceback，并预计算两侧 feature norm。生产实现与独立 full-matrix oracle 在随机小矩阵上 bit-exact；宽循环、norm、endpoint、traceback 与返回前都保留取消屏障。DP、14 维距离、边界精修和 exact assignment 仍在 CPU，4090 不会消除所有 CPU 占用。
  - 资源账本与事务边界闭环：短媒体 PCM 统一以 `max(container, selected stream) + 1s`、最多 60 分钟驱动 EOF stdout hard limit、frame count、最终制品和 phase 预算，不再用 `-t` 静默截断；raw stdout Vec、解析后 PCM、2 MiB stderr、streaming ring、fine `+50ms`、CUDA 两份 host output 与最坏 cuFFT device workspace 全部计费。60 分钟短媒体 CPU/CUDA 峰值分别为 `347697152` / `475788992` 字节，长媒体 CPU/CUDA-hybrid 峰值分别为 `16266826` / `200816202` 字节，CUDA fine 临时上界为 `176291840` 字节；统一要求“当前驻留 + max(最终制品、解码 phase、分析 phase) <= 1 GiB”。
  - 缓存感知 admission：同一 cache lock 下预取所有 Arc-backed hit，hit 只按实际驻留 payload 计费，miss 按 phase peak 与 final 上界计费；LRU 后续驱逐不会让已规划 hit 再解码。普通 source+target 会在首个 FFmpeg 前联合规划，target 以 source 的 projected retained 为前缀；若联合峰值越界，source decode closure 保持 0。批次继续按 distinct physical media 累计同一账本。独立最终复核确认无剩余 P0/P1。
  - 下一代多窗口 fine 的纯领域核心已建立：新增 `alignment-v2-adaptive-fine-frontier-v1`，包含有界候选库存、source/target 物理区间 occupancy、确定性 exact assignment、omitted optimistic upper bound、strict margin resolution、取消与 candidate/search/interval 资源上限，并以 exhaustive oracle 验证小规模结果。它当前只作为未接线的纯模块存在，尚未被 `audio_alignment` 生产调用；不得宣称第二次 exact assignment、pair-local 多窗口 fine competition 或 same-segment many-to-many 已上线，后续需以原子 Evidence v3 契约接入。
  - RTX 4090 环境无需安装：实测设备 `NVIDIA GeForce RTX 4090`、24564 MiB、driver `610.74`，CUDA Toolkit `13.3` / nvcc `13.3.73` 与 `bin\x64\cufft64_12.dll` 已存在，用户/机器 PATH 已持久配置。当前运行中的 Codex/Tauri 父进程继承的是旧环境；完全退出并重启后即可直接在“设置 → 播放器与工具 → NVIDIA CUDA/cuFFT 加速”检测和使用，无需重复安装。
  - 自动验收：Rust 串行全量 **356 passed / 18 ignored / 0 failed**，强制真实 RTX 4090 / CUDA/cuFFT **24/24**，资源预算定向 **11/11**，Rustfmt、strict Clippy `-D warnings` 与独立 P0/P1 复核通过。`corepack pnpm verify:release` 完整通过：源码审计、ESLint、Vitest **82 files / 823 tests**、TypeScript/Vite production build、Chromium 四页北极星 **5/5** 与 Tauri NSIS release。沙箱内首次 Vitest 因 esbuild 子进程 `spawn EPERM` 未启动，允许构建子进程后以相同命令完整通过，不是代码失败。
  - 本阶段安装器：`src-tauri/target/release/bundle/nsis/Danmaku Timeline Studio_0.1.0_x64-setup.exe`，大小 `4041611` 字节，时间 `2026-07-13 20:58:53 +08:00`，SHA-256 `F5C45470A152276736FAE41B8CF170752BA62DC1489EF26AC25EE8EEA784FF72`。
  - 本阶段便携版：`src-tauri/target/release/danmaku_timeline_studio.exe`，大小 `15841280` 字节，时间 `2026-07-13 20:58:53 +08:00`，SHA-256 `C2632FC97FA48EAF728081A62D11359F6A448700F4E8A4C749CB2E23DC5F3371`。
  - 当前多视频能力边界：普通产品已能一次提交最多 256 pair 的 N×M，并能处理互不重叠分集的 many-to-one / one-to-many；尚不能把同一物理片段的多个剪辑版本作为真正的 many-to-many 多窗口 fine 竞争。正式 evidence/receipt v2 仍绑定 coarse execution identity，fine actual backend 目前只进入诊断，需 Evidence v3 原子迁移；获授权真实冻结 gold、准确率与删减双边界标注仍未闭环，因此 C137 保持 active。
  - 本阶段 checkpoint 标签：`checkpoint/c137-cuda-fine-frontier-core-v1-20260713`。
