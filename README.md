...
- 导入参考视频（通常是 B 站删减版 MP4/WebM）和一个或多个 Bilibili XML 弹幕文件。
...
- 本地音频对齐依赖用户提供合法拥有或授权读取的完整版输入、B 站删减版参考输入和本机 FFmpeg；已绑定本地目标原片时会自动填入完整版输入，已绑定 Emby 目标原片时可用本次会话密码生成临时授权输入交给 FFmpeg。项目文件不会保存 Emby 密码、token 或播放 URL；网页模式不会伪装成本地桌面能力。
