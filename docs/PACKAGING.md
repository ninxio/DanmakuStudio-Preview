...
单次工具的 lifecycle Job 与 benchmark accounting Job 分工不同：前者负责进程树生命周期和有界清理，后者按精确成员集合提供 `windows-job-object-working-set-v1` 采样。strict raw v2 已能生成实际媒体卷、Job working-set 与终态 cleanup 回执；这些仍是应用内工程证据，只有在 authority v3 的动态进程 challenge、外部签名策略和正式协议同时成立时才能解除对应 release 门禁。
...
原生 N×M batch 另有完整 source-major receipt v4：除 source/target inventory、全部 pair、tile-local Top-10、shortlist 状态和 TimeMap 外，每个 pair 还在全局 assignment 与 fine 之前冻结固定版本的 intrinsic relation score，并绑定当前 native 可执行文件、固定 FFmpeg/FFprobe 和两端实际 spectral backend/fallback 的 path-free execution identity；selected pair 还绑定 fine frontier、实际双侧 decode window 和 proposal TimeMap digest。fine frontier contract v2 会公开每个 component 的完整 path-free `inventoryCandidates`，逐项记录 canonical pair/candidate ID、coarse upper bound 和构成该时间窗组的 relation members；`inventoryDigest` 与 receipt digest 双层绑定，删项、改窗、重编号或序号断层均失败关闭。completed receipt 只能声明一个由全部 pair 共同重算出的 identity digest；tile 内发生 Auto 回退分歧也会失败关闭。该原始 receipt 仍含内容身份和流元数据，只供本机内部协调器使用，不是可直接分享的匿名报告。普通产品和单个 native tile 每侧最多 256 个媒体、每次最多 256 pair。formal blind matrix v2 可把更大的 query×candidate 关系检索确定性拆成多个 tile，对完整 manifest 只预检一次，收齐每个 cell 后从全部 native relation score 重算 global Top-K；第一个 completed tile 固定 actual execution identity，任何缺失、重叠、参数漂移、执行身份漂移、候选同物理文件异流、receipt/job 重放或失败 tile 都不会生成 provenance。单批公开 accuracy 编译器继续要求本地候选全集完整，partial candidate shard 只能交给 exhaustive matrix 聚合器。视觉关闭时仅视频流不同不能冒充新候选，正式视觉模式则禁止 `null/auto` 流。公开组件结果仍固定为 `releaseEligible:false` / `untrusted-self-consistent-evidence`，不会仅因自洽 receipt 授予编辑/删减分类或边界准确度。私有 acceptance v6 会把 frozen case 唯一定位到 formal provenance 的 gold native pair，从完整 per-pair inventory 精确统计实际窗口候选，并从 selected execution 的实际双侧 decode window 与 proposal TimeMap 唯一重算 matched anchor、45 分钟漂移、编辑分类、持续时间及 source/target 四边界误差；TimeMap report 必须逐 case exact equal，修改数字后重签报告也会失败关闭。没有真实 frozen case 提供至少两个同 pair 窗口或多成员时间组时，相应多窗口和 same-segment many-to-many 检查仍会保持 incomplete。

同一高级区内的性能工程采集复用已导入 manifest 中的媒体引用，不会再要求选择第二组路径。native v2 在任何工具探测前校验 canonical blind manifest，为全部 distinct 媒体取得会话级只读 pin，只完整哈希一次，并从固定句柄探测实际卷；每个 job 只能使用注册 case 的配对和显式流。随后严格按 cold → warmup → hot → cancel 计划运行生产对齐核心。导出的 strict raw v2 保存 path-free `workloadStorage`、Rust 单调时钟、阶段边界、缓存计数、accounting Job working-set 和取消终态，并固定标记 `releaseEligible: false`、`trustStatus: "untrusted-raw-evidence"`。它不会保存路径、卷 GUID/serial 或单媒体 SHA，但稳定的 run manifest、workload、media-set 与 receipt 摘要仍可跨运行关联同一工作负载。组件报告里的协调器 wall time 不属于性能证据。

完整 release 验收另需严格的 `C137AcceptanceBundle` 和外部信任。bundle v6 / protocol v8 / relationship report v3 / calibration report v2 只接受 formal blind provenance v3 / plan v2、native evidence/receipt v4、fine frontier contract v2、固定 score contract、exhaustive matrix coverage 与“从全部 cell 重算 global Top-K”的聚合契约；report 的 scope、score、K 以及每条 `decisionId/provenanceRef/case/gold/rankedCandidateIds/verified` 都必须逐项等于 formal 重算。每条 calibration sample 还必须等于 formal Top-1/Top-2 原生分数及其规范化 margin，并通过 protocol 内获批、绑定独立 calibration split/dataset digest、覆盖 0..1、概率单调不减的分段线性模型唯一重算；caller 自填概率、改 margin、换模型摘要或使用 report v1 均失败关闭。manifest/gold/media/parameters/provenance root 分别绑定 bundle、dataset approval、preflight、runner 与 prediction receipt，公开聚合报告、单 tile Top-K 或旧 schema 不能代替该私有 envelope。performance report v3 仍只接受 formal raw schema v2，并独立检查实际媒体卷、Job memory、terminal cleanup 与 native attestation。

仓库外 P-256 authority v3 会在运行前签发一次性 challenge；桌面原生进程在内存中建立临时 Ed25519 会话，绑定 challenge/nonce、PID、Windows 进程创建 FILETIME、当前 EXE 全文件摘要和临时公钥。formal blind batch 与性能 session 只有真实进入原生终态后才能各封存一次 `nativeJobId/receiptDigest` 或 `collector sessionId/evidenceDigest`；finalize 必须完整覆盖预注册 inventory，再用同一临时私钥签名动态 evidence root。`attest` 会独立通过 Windows 查询该 PID 的启动时间和映像路径、重新哈希 live image、复核 Authenticode signer，并把进程观察、动态回执、bundle 和 append-only ledger 一起签名。跨进程替换、漏封、重复封存、终态前封存、重放、字段漂移和 authority v2 降级均失败关闭。该软件证明已经能解除 blind/performance 的动态 native attestation 检查，但它不是 TPM/AIK 或硬件不可迁移密钥证明。

当前仍没有正式 provision 的 production authority key/policy、受信代码签名证书和获批真实 calibration 模型。安装包默认不携带可自行签发的私钥，当前 release 也未签名，因此无法用工程包自我晋级，正式结果继续是 `incomplete-evidence`。

release 不携带真实媒体、gold、许可材料或 raw 性能记录。当前仓库已能生成 development Gold 候选及治理 bundle，也已完成实际媒体卷、Job working-set、终态 cleanup、软件 live-process attestation 和仓库外 authority 的工程证据链；但这些材料固定不可自行晋升为 frozen。正式证据仍需获授权的真实 Gold、production key/policy/signer、批准的 calibration split/model、规定 4 核目标机重复测量和 20 套真实北极星长合集，再按获批协议重新采集。
...
- 签名后的候选 EXE 可先运行 `corepack pnpm c137:authority -- inspect-native --native-executable <signed.exe>` 取得 signer 证书摘要；初始化仓库外 authority v3 时必须用 `--native-signer-cert-sha256 sha256:<64-hex>` 固定允许的叶证书。运行时需要先签发 challenge，再由该桌面进程完成 begin → 原生任务封存 → finalize 并保存 live-process receipt；`attest` 必须额外传入 `--live-process-attestation <receipt.json>` 和 `--native-executable <signed.exe>`，`verify` 也必须重新提供同一候选 EXE。当前未签名产物会失败关闭。
- authority v3 的临时 Ed25519 + Windows PID/启动时间/映像检查是软件 live-process attestation，不等于 TPM/AIK、硬件不可迁移密钥或受保护进程。它能关闭当前动态证据断点，但不能单独把未 provision、未签名、无真实冻结数据的工程预览晋级为正式准确率或性能证明。
