...
- 本地音频对齐实验室：在 Tauri 桌面端调用用户本机 FFmpeg 抽取音频特征，生成 `AlignmentProposal`，支持后台进度、取消、任务日志、调参、导入/导出提案和一键应用。
- Emby 时长辅助：通过设置中心保存非敏感连接项，资源栏搜索用户授权的 Emby 媒体条目并导入真实集时长规则；本次会话密码只保存在内存。
...
5. 导出 XML 前查看摘要和补偿报告；导出的 XML 会重新解析验证，原始 XML 不会被直接修改。
...
- 当前对齐算法是本地音频特征和动态规划候选生成，仍需要人工复核；候选区精扫和视觉对齐仍是后续增强。
- Emby 集成只处理用户配置且有权限访问的媒体库元数据，不实现视频下载、DRM 绕过、账号绕过、私有接口爬取或未授权媒体访问。
- Emby 密码和 token 不写入项目文件或本地设置；本次会话密码关闭应用后失效。
...
- 增加候选区精扫，提高真实视频删减点边界精度。
- 扩展 Playwright E2E 覆盖导出 XML 后重新导入、复杂编辑撤销恢复和更多窄窗口截图。
- 将设置存储从浏览器 localStorage 迁移到 Tauri 应用配置目录，并评估系统凭证库保存敏感凭证。
