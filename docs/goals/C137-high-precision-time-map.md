...
- [ ] 将同一 live native process 接入 challenge-response/OS 进程映像或 TPM/AIK attestation，并把 modality/calibration 纳入冻结审批；仅完成 Authenticode artifact 绑定，在此之前 `native-blind-native-attestation` 与性能 `native-attestation` 继续 incomplete。
