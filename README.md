...
- 项目信息页：提供项目健康摘要，提示重复 ID、缺失资源引用、负最终时间、空片段、媒体重连、导入警告、低置信锚点和失效编辑引用等保存/重开/导出前风险，重复 ID、缺失资源片段、导入警告、失效编辑引用和负最终时间会列出具体出现位置，证据过长时提示剩余数量，可下载健康报告，并可一键清理指向不存在资源的片段和指向不存在弹幕的失效禁用/微调引用。
...
资源栏“项目信息”会基于当前运行时状态生成项目健康摘要，用于在保存、重开或导出前快速复核媒体重连、空片段、导入警告、重复 ID、负最终时间和失效引用；重复 ID、缺失资源片段、导入警告、失效编辑引用与负最终时间会显示出现位置并写入可下载健康报告，证据列表过长时会提示剩余未列出数量，其中缺失资源片段、失效禁用/单条微调引用可通过真实清理动作进入历史栈，仍可撤销。
