...
当前已完成 production blind benchmark runner、组件级 TimeMap 评测降权、C137 acceptance bundle v2 / protocol v4 的 fail-closed 聚合器、A/B 播放复核证据 v2，以及 native/raw/acceptance 性能证据链第五阶段。私有 formal blind provenance v1 已把完整 manifest、预注册 query shards、完整 candidate universe、execution suite、native receipt、确定性 raw/aggregate 与逐 case ranking 接入 acceptance；旧式调用方 Top-K 不再能只靠重签报告形成内容闭环。该 provenance 仍明确是不受信自洽证据，外部 plan authority、native execution attestation、challenge freshness、防重放账本和 modality/calibration 来源尚未完成。性能链继续使用 performance report v3 / raw schema v2，能从生产调用链记录 session-relative 单调时钟、真实阶段边界、冷/热缓存、取消终态和应用进程树 working set，并在任何工具探测前固定全部 workload media、逐 distinct 文件完整哈希、按实际卷生成 path-free storage receipt。Windows 一次性媒体工具仍由独立 lifecycle Job Object 挂起创建、入 Job 后恢复并有界清理；普通对齐 run 的 source/target media lease、run-start/run-final SHA、TimeMap identity、frame/packet PTS、缓存和音画证据继续绑定同一全文件身份。
...
完整 C137 acceptance bundle 已定义 versioned protocol、数据审批/preflight/prediction receipts，以及 dataset、关系排名、TimeMap、校准、视觉回退、安全降级、北极星、性能、UI 和 release 原始报告。bundle v2 / protocol v4 / relationship report v2 另有私有 formal blind structural provenance：plan 在运行前锁定完整 candidate universe、只切 query 轴的 shards、axis、visual、K 和参数摘要；acceptance 从完整 manifest 重建 projection，严格复核 execution 路径、全文件身份、流、full-Cartesian pairs、completed receipt、raw、aggregate 与 provenance root。每个 frozen case 必须全局恰好一次，relationship report 的 `decisionId/provenanceRef/case/gold/Top-K/verified` 必须逐条等于派生结果。performance report v3 继续只接受 raw schema v2，并检查实际媒体卷、Job memory、terminal cleanup 与 native attestation。旧 bundle v1/protocol v3/ranking v1、raw v1 和自建 trustContext 均不能晋级。内容自洽不等于来源权威；缺 plan/native authority、challenge/freshness/replay ledger、modality/calibration provenance 或性能 assurance 时仍只能得到 `incomplete-evidence`。
...
- [x] acceptance bundle v2 / protocol v4 与 performance report v3 只接受对应 formal schema；性能链独立检查当前 manifest、storage、Job memory、terminal cleanup 与 attestation，blind 结构链直接重算 plan/projection/execution/receipt/raw/decision；改字符串、自摘要、选择性漏报、重复 case、自建 trustContext 或跨 workload 重签均不能通过。
...
- [x] formal acceptance 直接消费并重算私有 full-Cartesian blind plan/projection/execution/native-receipt/raw/aggregate provenance，并逐条绑定 relationship report；调用方自报 Top-K 不能形成内容闭环。
...
- [ ] 解除当前每侧 64 distinct candidate 的 formal 容量阻断，或由 authority 定义不可挑选的冻结 cohort；禁止把多个候选分片的局部 Top-K 拼成全局准确率。
- [ ] pair-local 多窗口 Top-K、编辑/删减分类 F1、双侧边界误差和同一片段多版本 many-to-many fine alignment 进入同一冻结 gold 证据链；当前跨媒体关系 Top-K 不得冒充这些指标。

当前剩余限制：逐段真实留出证据、双侧边界不确定范围、A/B 人工复核、blind runner、lifecycle Job、工程 raw v2、实际媒体卷回执和 fail-closed acceptance v4 虽已落地，但获授权且实际运行的真实冻结关系数仍为 0；概率仍必须保持 null，实测校准、批准的 production protocol / trust root、blind plan/native authority、Job working-set/terminal cleanup/native attestation 正式证据和 20 套北极星长合集均未完成。candidate side 超过 64 distinct media 时 formal v1 也会诚实阻断，不能据此宣称准确率或性能达标。
...
N×M blind 组件证据已进一步接入私有 formal structural provenance。native snapshot 对全部 pair 输出 rank/score/margin、候选总数、Top-10 和 shortlist；纯领域 contract 严格复核计数、rank、TimeMap identity、execution inventory 与 source-major 顺序。`caseIds` 只切 query 轴，`candidateCaseIds` 在每个 shard 中保持完整候选全集；suite digest 同时绑定两者。acceptance v2 保存完整 manifest、plan、execution suite、native receipt、raw 与 aggregate，并从 receipt 唯一派生逐 case decision；每个 case 全局只能计数一次，同一 Windows 规范化路径也不能跨 shard 绑定不同 full-file identity，路径、完整身份、显式流、参数、gold 和 provenance root 都有独立 binding。公开 DTO 仍仅含聚合指标，私有 formal v1 也固定为 `releaseEligible:false` / `untrusted-self-consistent-provenance`。当前每侧 64、总计 256 pair 的上限仍会拒绝 distinct candidate 超过 64 的完整全集；authority/native attestation 与 pair-local window/edit/boundary 证据也尚未完成，所以这条接线不能冒充 release authority。
