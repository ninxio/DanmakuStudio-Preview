...
- 当前 V2 已有 PTS、多音轨、声谱 landmark、速度漂移、双向编辑、局部边界、独立视觉回退和版本化原生分数校准契约，但真实冻结集样本数仍为 **0**，benchmark manifest v2 示例只有 placeholder。尚未用获批真实 calibration split 拟合/批准生产模型，也未完成规定硬件性能报告或 20 套北极星长合集 5/5 验收。自动结果最高为 `review`；正式按原片分集导出只接受带可信验证来源的 `verified` 时间图。因此本阶段 release 是安全预览，不代表准确率或性能验收完成。
