...
完整 release 判定由 `src/domain/alignment/c137Acceptance.ts` 的严格 `C137AcceptanceBundle` 单独负责。bundle v2 / protocol v4 / relationship report v2 新增私有 `formalEvidence.blindRelationship`：协议锁定 blind plan 与参数摘要，report decision 新增可空 `provenanceRef`；`null` 表示 legacy/self-reported，只能 incomplete。`src/domain/alignment/c137FormalBlindProvenance.ts` 保存完整冻结 manifest、query-axis shards、完整 candidate universe、execution suite、native receipt、raw 与 aggregate evidence，并从每个 completed full-Cartesian receipt 确定性重建逐 case gold/Top-K。validator 要求 plan/envelope 按序双射、每个 frozen case 全局恰好一次、候选全集不得裁剪、同一 Windows 规范化路径在完整 manifest 中只能绑定一份 full-file identity，projection/execution 的路径、全文件身份、显式流、pairs 与参数全部闭合；prediction receipt 和全部 report 再绑定同一 provenance root。关系报告的 `decisionId/provenanceRef/case/gold/ranking/verified` 必须逐条等于派生结果，调用方重排后重签不会通过。

这条 formal v1 仍固定为 `releaseEligible:false` / `untrusted-self-consistent-provenance`。它关闭的是内容自洽与漏报/重放边界，不证明 native job 或签发者真实存在；外部 plan authority、native execution attestation、challenge freshness、防重放账本以及 modality/calibration 来源仍各自 incomplete，`external-trust-authority` 也不变。performance report v3 继续只接受 raw schema v2，并检查 runManifest、workload storage、Job memory、terminal cleanup 与 attestation。旧 bundle v1/protocol v3/ranking v1 和 raw v1 均不能混入新语义。当前 candidate side 仍受 64 distinct media / 256 pair 上限约束；完整候选全集超过 64 时会失败关闭，不能把候选分片的局部 Top-K 拼成全局证据。
...
原生 batch snapshot 的 evidence v1 固定报告 `fullCartesian`、source/target 有序 inventory、source-major pair index，以及每个 pair 的全候选数、eligible 数、Top-10、decision candidate、rank/score/margin 和 shortlist 状态。coarse shortlist 和 fine proposal 都先保留在 worker 私有 staging；最终媒体与工具身份复核通过后才一次发布，失败会清空整批 proposal/选择结果，取消只保留已经完成并复核的 pair。TypeScript bridge 对 exact keys、pair index、候选计数、Top-10 截断、decision rank 内容和 TimeMap identity 做严格闭合，不接受前端补造的简化 snapshot。

`c137BlindBatchEvidence.ts` 与 `c137BlindBatchBenchmark.ts` 提供独立的 blind N×M 组件证据链，`realMediaBlindBatchContract.ts` 则承载不依赖 Tauri 的 execution/receipt exact schema、digest 与确定性 ranking validator。调用方必须显式声明关系查询轴与实际视觉模式；`caseIds` 只选择 query/decision，`candidateCaseIds` 独立选择候选全集并必须包含 query，两个数组都唯一且保持 manifest 顺序。source 轴只切 source query，target 轴只切 target query；candidate inventory 仍完整进入每个 shard。媒体两侧分别用 manifest/domain salted commitment 排列，私有 projection 绑定完整内容身份和实际消费流但不暴露原始摘要或路径，候选侧 distinct 数必须严格大于 K。compiler 与 runner/native 共用每侧 64、总计 256 pair 的硬边界，并在任何路径 preflight 前拒绝超限 suite。视觉关闭时 video stream 被归一为 `null`，不能靠未消费的视频流虚增候选；formal visual 模式要求每个 selected source/target 预先固定非空视频流，禁止 `null/auto`。协调器先完成路径/身份/流 preflight，再发起一次完整 native batch；只有整批和每个 pair 都 completed 的 receipt 通过 strict contract 后，领域函数才唯一派生 raw/ranking 并揭示冻结 gold。任何 partial、失败、取消、receipt ranking 篡改或 projection 漂移都令 accuracy evidence 为空。公开 DTO 仍只有聚合计数、准确率与误差分布，不能当作 native 签名或 release authority；v1 也不评测同一 pair 内多窗口 Top-K、编辑分类、删减/插入 F1 或边界误差。
