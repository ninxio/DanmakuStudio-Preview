...
完整 release 验收另需严格的 `C137AcceptanceBundle` 和外部信任。bundle v5 / protocol v7 / relationship report v3 只接受 formal blind provenance v3 / plan v2、native evidence/receipt v4、fine frontier contract v2、固定 score contract、exhaustive matrix coverage 与“从全部 cell 重算 global Top-K”的聚合契约；report 的 scope、score、K 以及每条 `decisionId/provenanceRef/case/gold/rankedCandidateIds/verified` 都必须逐项等于 formal 重算。manifest/gold/media/parameters/provenance root 分别绑定 bundle、dataset approval、preflight、runner 与 prediction receipt，公开聚合报告、单 tile Top-K 或旧 schema 不能代替该私有 envelope。performance report v3 仍只接受 formal raw schema v2，并独立检查实际媒体卷、Job memory、terminal cleanup 与 native attestation。仓库外 P-256 authority 已能签发一次性 challenge、原子消费 append-only ledger 并签名最终 bundle；但当前仍没有正式 provision 的 production key/policy，native execution 也没有 Authenticode/TPM attestation，modality/calibration 来源未闭合。安装包默认不携带可自行签发的私钥，因此正式结果继续是 `incomplete-evidence`。
