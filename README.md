...
当前项目文件 schema 为 v6。v2 片段源区间使用半开语义 `[sourceInMs, sourceOutMs)`；v3 保存当前对齐提案；v4 新增 `mediaBinding`，用于保存本地文件或 Emby 条目的项目级目标原片引用；v5 新增 `seasonEpisodeBindings`，用于保存剧集批量工作台中每个输出集对应的目标原片引用；v6 新增 `danmakuSourceSegments`，用于保存 B 站/XML 时间轴上的虚拟内容段和忽略范围。打开 v1-v5 项目时会自动迁移到当前 schema，保存项目时始终写入当前 schemaVersion。
...
- 左侧资源面板：默认显示弹幕素材、目标原片绑定、匹配评分、下一步操作和导出检查；高级工具展开后可使用 Emby 时长、人工整理规则、弹幕来源内容段、逐集目标绑定、版本差异列表、疑似版本差异扫描、锚点校准、同步线索管理和视频对齐实验室。
