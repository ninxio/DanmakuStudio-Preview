...
- 本地音频对齐实验室：在 Tauri 桌面端调用用户本机 FFmpeg 抽取音频特征，生成 `AlignmentProposal`，支持后台进度、取消、任务日志、调参、目标本地路径复用、进程内音频特征缓存、音频候选边界细化、不确定区间展示、候选版本差异逐条预览/修正/接受/跳过、导入提案、按项目名导出提案和复核报告、应用前复核提示、带具体 ID 的异常提案应用阻断和一键应用。
...
- 播放预览：HTML Video 作为 MP4/WebM 轻量 fallback；桌面端配置 mpv 且目标原片有真实本地路径时可切到 mpv sidecar。预览会显示未导入、正在载入、可播放、格式不支持和需要重新连接等状态，支持播放/暂停、seek、弹幕叠加、透明度调整，并可在当前播放点直接标记版本差异。
- 撤销、重做和常用快捷键。
...
- 本地音频对齐依赖用户提供合法拥有或授权读取的完整版、当前视频和本机 FFmpeg；已绑定本地目标原片时会自动填入完整版路径，已绑定 Emby 目标原片时仍需要用户选择可读取的本地文件路径，网页模式不会伪装成本地桌面能力。
...
- 完善音频对齐实用化：把进程内特征缓存扩展为可管理的持久缓存；验证 Emby 授权流或低码率转码采样后，再接入 Emby 目标原片作为直接输入。
